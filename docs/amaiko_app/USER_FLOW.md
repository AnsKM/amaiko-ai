# USER_FLOW.md - User Flow Documentation

**Version:** 1.0
**Last Updated:** November 2025
**Application:** Amaiko AI - Enterprise AI Assistant

---

## Table of Contents

1. [Overview](#overview)
2. [Employee Onboarding Flow](#employee-onboarding-flow)
3. [Daily Usage Scenarios](#daily-usage-scenarios)
4. [Email Automation Flows](#email-automation-flows)
5. [Calendar Management Flows](#calendar-management-flows)
6. [Document Search Flows](#document-search-flows)
7. [Meeting Summary Flows](#meeting-summary-flows)
8. [Knowledge Base Interaction Flows](#knowledge-base-interaction-flows)
9. [Workflow Automation Flows](#workflow-automation-flows)
10. [Error Handling and Recovery Flows](#error-handling-and-recovery-flows)
11. [Multi-Agent Collaboration Flows](#multi-agent-collaboration-flows)
12. [Proactive Assistance Flows](#proactive-assistance-flows)

---

## Overview

This document describes the comprehensive user flows for the Amaiko AI enterprise assistant application. Each flow describes the step-by-step interaction between users and the AI assistant through Microsoft Teams, including authentication, tool usage, multi-agent collaboration, and error handling.

### Key User Interaction Patterns

```
┌─────────────────────────────────────────────────────────────┐
│                    User Interaction Types                   │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. Direct Commands    → Explicit user instructions         │
│  2. Natural Language   → Conversational requests            │
│  3. Proactive Alerts   → System-initiated notifications     │
│  4. Workflow Triggers  → Multi-step automated processes     │
│  5. Knowledge Queries  → Information retrieval requests     │
│  6. Context-aware      → Implicit intent recognition        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### User Journey Overview

```
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│              │      │              │      │              │
│  Onboarding  │─────▶│ Daily Usage  │─────▶│   Advanced   │
│              │      │              │      │   Features   │
│              │      │              │      │              │
└──────────────┘      └──────────────┘      └──────────────┘
       │                     │                     │
       │                     │                     │
       ▼                     ▼                     ▼
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│ - First login│      │ - Email help │      │ - Workflows  │
│ - Auth setup │      │ - Calendar   │      │ - Multi-agent│
│ - Tutorial   │      │ - Search docs│      │ - Analytics  │
└──────────────┘      └──────────────┘      └──────────────┘
```

---

## Employee Onboarding Flow

### Flow 1.1: First-Time User Setup

**Objective:** Set up new employee with Amaiko AI assistant and establish authentication.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│  installs   │
│ Teams App   │
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Initial Welcome Message                         │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ╔════════════════════════════════════════════════════╗ │
│  ║  👋 Welcome to Amaiko AI!                          ║ │
│  ║                                                    ║ │
│  ║  I'm your personal AI assistant for Microsoft 365  ║ │
│  ║                                                    ║ │
│  ║  I can help you with:                              ║ │
│  ║  • Email management                                ║ │
│  ║  • Calendar scheduling                             ║ │
│  ║  • Document search                                 ║ │
│  ║  • Meeting summaries                               ║ │
│  ║  • Knowledge base queries                          ║ │
│  ║                                                    ║ │
│  ║  [Get Started] [Learn More]                        ║ │
│  ╚════════════════════════════════════════════════════╝ │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ User clicks "Get Started"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Authentication Setup                            │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ╔════════════════════════════════════════════════════╗ │
│  ║  🔐 Connect Your Microsoft Account                 ║ │
│  ║                                                    ║ │
│  ║  To access your emails, calendar, and files,       ║ │
│  ║  please sign in with your Microsoft account.       ║ │
│  ║                                                    ║ │
│  ║  Permissions needed:                               ║ │
│  ║  ✓ Read and manage emails                          ║ │
│  ║  ✓ Access calendar                                 ║ │
│  ║  ✓ Search files in OneDrive/SharePoint             ║ │
│  ║                                                    ║ │
│  ║  [Sign In with Microsoft] [Learn About Security]   ║ │
│  ╚════════════════════════════════════════════════════╝ │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ User clicks "Sign In"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Azure AD OAuth 2.0 Flow                         │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  1. User is redirected to Azure AD login page           │
│  2. User enters Microsoft 365 credentials               │
│  3. Consent screen displays requested permissions       │
│  4. User grants consent                                 │
│  5. Azure AD returns authorization code                 │
│  6. Backend exchanges code for access token             │
│  7. Token stored securely in agent mapping              │
│                                                         │
│  [Backend Process Flow]                                 │
│                                                         │
│  ┌─────────────┐                                        │
│  │ User grants │                                        │
│  │   consent   │                                        │
│  └──────┬──────┘                                        │
│         │                                               │
│         ▼                                               │
│  ┌─────────────────────────────────┐                    │
│  │ AgentService.createUserAgent()  │                    │
│  │                                 │                    │
│  │ 1. Create Letta agent           │                    │
│  │ 2. Initialize memory blocks     │                    │
│  │ 3. Save agent mapping to DB     │                    │
│  │ 4. Register user preferences    │                    │
│  └─────────────────────────────────┘                    │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ Agent created successfully
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Personalization Setup                           │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ╔════════════════════════════════════════════════════╗ │
│  ║  ✅ You're all set!                                ║ │
│  ║                                                    ║ │
│  ║  Let me learn a bit about you to provide better    ║ │
│  ║  assistance:                                       ║ │
│  ║                                                    ║ │
│  ║  1. What's your role?                              ║ │
│  ║     [Sales] [Engineering] [Marketing] [Other]      ║ │
│  ║                                                    ║ │
│  ║  2. What's your typical work schedule?             ║ │
│  ║     [9-5] [Flexible] [Remote]                      ║ │
│  ║                                                    ║ │
│  ║  3. What matters most to you?                      ║ │
│  ║     [ ] Inbox zero                                 ║ │
│  ║     [ ] Meeting efficiency                         ║ │
│  ║     [ ] Quick document access                      ║ │
│  ║                                                    ║ │
│  ║  [Skip] [Continue]                                 ║ │
│  ╚════════════════════════════════════════════════════╝ │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ User provides preferences
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 5: Memory Block Initialization                     │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Backend: AgentMemoryService.updateAgentMetadata()      │
│                                                         │
│  ┌─────────────────────────────────────────┐            │
│  │ Persona Memory Block                    │            │
│  ├─────────────────────────────────────────┤            │
│  │ name: "Amaiko Assistant"                │            │
│  │ role: "Professional AI Assistant"       │            │
│  │ tone: "Professional, friendly, helpful" │            │
│  │ capabilities: [...]                     │            │
│  └─────────────────────────────────────────┘            │
│                                                         │
│  ┌─────────────────────────────────────────┐            │
│  │ Human Memory Block                      │            │
│  ├─────────────────────────────────────────┤            │
│  │ name: "John Doe"                        │            │
│  │ role: "Sales Manager"                   │            │
│  │ department: "Sales"                     │            │
│  │ preferences: {                          │            │
│  │   workSchedule: "9-5",                  │            │
│  │   priorities: ["inbox_zero"]            │            │
│  │ }                                       │            │
│  └─────────────────────────────────────────┘            │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ Preferences saved
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 6: Interactive Tutorial                            │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ╔═══════════════════════════════════════════════════╗  │
│  ║  🎓 Quick Tutorial                                ║  │
│  ║                                                   ║  │
│  ║  Try these commands to get started:               ║  │
│  ║                                                   ║  │
│  ║  1️⃣ "Search my recent emails"                     ║  │
│  ║     → Finds emails from the past week             ║  │
│  ║                                                   ║  │
│  ║  2️⃣ "What's on my calendar today?"                ║  │
│  ║     → Shows today's meetings                      ║  │
│  ║                                                   ║  │
│  ║  3️⃣ "Find documents about Q4 planning"            ║  │
│  ║     → Searches OneDrive and SharePoint            ║  │
│  ║                                                   ║  │
│  ║  Or just chat naturally - I understand context!   ║  │
│  ║                                                   ║  │
│  ║  [Try Command 1] [Try Command 2] [Start Using]    ║  │
│  ╚═══════════════════════════════════════════════════╝  │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ User selects "Start Using"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 7: Onboarding Complete                             │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ╔═══════════════════════════════════════════════════╗  │
│  ║  🎉 You're ready to go!                           ║  │
│  ║                                                   ║  │
│  ║  I'm here to help anytime. Just type a message    ║  │
│  ║  or use the menu for quick commands.              ║  │
│  ║                                                   ║  │
│  ║  💡 Pro tip: I learn from our conversations,      ║  │
│  ║  so I'll get better at helping you over time!     ║  │
│  ║                                                   ║  │
│  ║  [View Help] [Dismiss]                            ║  │
│  ╚═══════════════════════════════════════════════════╝  │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/agent.service.ts
async createUserAgent(userId: string, userProfile: any): Promise<string> {
  // 1. Create Letta agent with initial configuration
  const agent = await this.lettaClient.createAgent({
    name: `agent-${userId}`,
    memory: {
      persona: this.buildPersonaMemory(),
      human: this.buildHumanMemory(userProfile),
      conversationContext: '',
      archival: [],
    },
    tools: this.getAvailableTools(),
    model: 'gpt-4o',
    contextWindow: 128000,
  });

  // 2. Save agent mapping to database
  await this.agentMemoryService.saveAgentMapping({
    userId,
    agentId: agent.id,
    metadata: {
      createdAt: new Date(),
      userProfile,
      preferences: {},
    },
  });

  // 3. Return agent ID
  return agent.id;
}
```

**Database Records Created:**

```sql
-- agent_mappings table
INSERT INTO agent_mappings (id, user_id, agent_id, metadata, created_at)
VALUES (
  'uuid-1',
  'user-123',
  'letta-agent-456',
  '{
    "userProfile": {"name": "John Doe", "role": "Sales Manager"},
    "preferences": {"workSchedule": "9-5", "priorities": ["inbox_zero"]}
  }',
  '2025-11-03 10:00:00'
);
```

---

### Flow 1.2: Returning User Login

**Objective:** Authenticate returning user and restore agent context.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│ opens Teams │
│     App     │
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Check Existing Agent Mapping                    │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Backend: AgentMemoryService.getAgentId(userId)         │
│                                                         │
│  Query: SELECT agent_id FROM agent_mappings             │
│         WHERE user_id = 'user-123'                      │
│                                                         │
│  Result:                                                │
│  ┌─────────────────────────────────┐                    │
│  │ agent_id: 'letta-agent-456'     │                    │
│  │ status: 'active'                │                    │
│  │ last_active: '2025-11-02'       │                    │
│  └─────────────────────────────────┘                    │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ Agent found
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Token Validation                                │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Check if user's Microsoft Graph token is valid:        │
│                                                         │
│  ┌────────────────────────────────────────┐             │
│  │ Token Status Check                     │             │
│  ├────────────────────────────────────────┤             │
│  │ IF token.expiresAt > Date.now()        │             │
│  │    → Token valid, proceed              │             │
│  │ ELSE IF token.refreshToken exists      │             │
│  │    → Refresh token automatically       │             │
│  │ ELSE                                   │             │
│  │    → Prompt user to re-authenticate    │             │
│  └────────────────────────────────────────┘             │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ Token valid
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Load Agent Context                              │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Backend: Load agent's memory blocks from Letta         │
│                                                         │
│  ┌─────────────────────────────────────────┐            │
│  │ Memory Blocks Loaded                    │            │
│  ├─────────────────────────────────────────┤            │
│  │ • Persona: Assistant personality        │            │
│  │ • Human: User profile and preferences   │            │
│  │ • Conversation: Recent chat history     │            │
│  │ • Archival: Long-term knowledge (5 msgs)│            │
│  └─────────────────────────────────────────┘            │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ Context loaded
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Welcome Back Message                            │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  👋 Welcome back, John!                           ║  │
│  ║                                                    ║  │
│  ║  📅 You have 3 meetings today                     ║  │
│  ║  📧 5 unread emails in your inbox                 ║  │
│  ║                                                    ║  │
│  ║  What would you like to focus on first?           ║  │
│  ║                                                    ║  │
│  ║  [Check Calendar] [Review Emails] [Continue Chat] ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/agent-memory.service.ts
async getAgentId(userId: string): Promise<string | null> {
  const mapping = await this.agentMappingRepository.findOne({
    where: { userId, isActive: true },
  });

  if (!mapping) {
    return null;
  }

  // Check if agent still exists in Letta
  try {
    await this.lettaClient.getAgent(mapping.agentId);
    return mapping.agentId;
  } catch (error) {
    // Agent doesn't exist, deactivate mapping
    mapping.isActive = false;
    await this.agentMappingRepository.save(mapping);
    return null;
  }
}
```

---

## Daily Usage Scenarios

### Flow 2.1: Morning Briefing

**Objective:** Provide user with daily summary of emails, calendar, and tasks.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│ types "good │
│  morning"   │
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Intent Recognition                              │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Letta Agent analyzes user message:                     │
│                                                           │
│  Input: "good morning"                                   │
│  Intent: DAILY_BRIEFING                                  │
│  Confidence: 0.95                                        │
│  Required Tools: [                                       │
│    'get_calendar_events',                               │
│    'search_emails',                                      │
│    'get_user_profile'                                    │
│  ]                                                       │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ Intent identified
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Parallel Tool Execution                         │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent executes multiple tools concurrently:            │
│                                                           │
│  ┌─────────────────────────────────────┐                │
│  │ Tool 1: get_calendar_events         │                │
│  │ Params: {                           │                │
│  │   startDate: "2025-11-03",          │                │
│  │   endDate: "2025-11-03"             │                │
│  │ }                                   │                │
│  │ Status: ✓ Complete (250ms)          │                │
│  └─────────────────────────────────────┘                │
│                                                           │
│  ┌─────────────────────────────────────┐                │
│  │ Tool 2: search_emails               │                │
│  │ Params: {                           │                │
│  │   query: "unread",                  │                │
│  │   maxResults: 10                    │                │
│  │ }                                   │                │
│  │ Status: ✓ Complete (180ms)          │                │
│  └─────────────────────────────────────┘                │
│                                                           │
│  ┌─────────────────────────────────────┐                │
│  │ Tool 3: get_user_profile            │                │
│  │ Params: {}                          │                │
│  │ Status: ✓ Complete (120ms)          │                │
│  └─────────────────────────────────────┘                │
│                                                           │
│  Total Time: 450ms (parallel execution)                 │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ All tools complete
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Response Synthesis                              │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent synthesizes results into coherent response:      │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  Good morning, John! ☀️                           ║  │
│  ║                                                    ║  │
│  ║  Here's your daily briefing:                      ║  │
│  ║                                                    ║  │
│  ║  📅 CALENDAR (3 events today)                     ║  │
│  ║  • 9:00 AM - Team Standup (30 min)               ║  │
│  ║  • 11:00 AM - Client Demo - Acme Corp (1 hr)     ║  │
│  ║  • 2:00 PM - Sprint Planning (2 hrs)             ║  │
│  ║                                                    ║  │
│  ║  📧 INBOX (5 unread emails)                       ║  │
│  ║  • Sarah: Q4 Budget Review - URGENT              ║  │
│  ║  • Mike: Follow-up from yesterday's meeting       ║  │
│  ║  • HR: Benefits enrollment deadline approaching   ║  │
│  ║  • ... and 2 more                                 ║  │
│  ║                                                    ║  │
│  ║  💡 SUGGESTIONS                                    ║  │
│  ║  • You might want to review Sarah's budget email  ║  │
│  ║    before the 11 AM client demo                   ║  │
│  ║  • Your client demo materials are in the folder   ║  │
│  ║    "Q4-Demos" in OneDrive                         ║  │
│  ║                                                    ║  │
│  ║  [Review Urgent Emails] [Check Demo Materials]    ║  │
│  ║  [View Full Calendar]                             ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ User clicks "Review Urgent Emails"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Follow-up Action                                │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent executes search_emails with filter:              │
│                                                           │
│  Params: {                                               │
│    query: "from:sarah urgent",                           │
│    maxResults: 5                                         │
│  }                                                       │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  📧 Email from Sarah Johnson                      ║  │
│  ║                                                    ║  │
│  ║  From: sarah.johnson@company.com                  ║  │
│  ║  Subject: Q4 Budget Review - URGENT               ║  │
│  ║  Received: 8:45 AM                                ║  │
│  ║                                                    ║  │
│  ║  Hi John,                                         ║  │
│  ║                                                    ║  │
│  ║  We need to finalize Q4 budget allocations        ║  │
│  ║  before today's client demo. Can you review       ║  │
│  ║  the attached spreadsheet and confirm by 10 AM?   ║  │
│  ║                                                    ║  │
│  ║  Thanks,                                          ║  │
│  ║  Sarah                                            ║  │
│  ║                                                    ║  │
│  ║  📎 Attachment: Q4-Budget-Draft.xlsx              ║  │
│  ║                                                    ║  │
│  ║  [Reply] [Download Attachment] [Mark as Read]     ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/agent.service.ts
async handleDailyBriefing(
  userId: string,
  context: McpContext,
): Promise<string> {
  // Execute tools in parallel for faster response
  const [calendarEvents, unreadEmails, userProfile] = await Promise.all([
    this.executeTool('get_calendar_events', {
      startDate: new Date().toISOString().split('T')[0],
      endDate: new Date().toISOString().split('T')[0],
    }, context),
    this.executeTool('search_emails', {
      query: 'unread',
      maxResults: 10,
    }, context),
    this.graphService.getUserProfile(context.userToken),
  ]);

  // Synthesize briefing response
  const briefing = {
    greeting: `Good morning, ${userProfile.displayName}!`,
    calendar: this.formatCalendarSummary(calendarEvents),
    inbox: this.formatEmailSummary(unreadEmails),
    suggestions: this.generateSmartSuggestions(calendarEvents, unreadEmails),
  };

  return this.formatBriefingResponse(briefing);
}
```

---

### Flow 2.2: Quick Email Check

**Objective:** Search and retrieve specific emails quickly.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│ types query │
└──────┬──────┘
       │ "Show me emails from Sarah about budget"
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Query Parsing                                   │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Letta Agent extracts search parameters:                │
│                                                           │
│  Raw Input: "Show me emails from Sarah about budget"    │
│                                                           │
│  Parsed Parameters:                                      │
│  ┌─────────────────────────────────────┐                │
│  │ sender: "Sarah"                     │                │
│  │ keywords: ["budget"]                │                │
│  │ timeframe: null (default: 30 days) │                │
│  │ folder: null (default: all)         │                │
│  │ maxResults: 10 (default)            │                │
│  └─────────────────────────────────────┘                │
│                                                           │
│  Tool Selected: search_emails                            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Microsoft Graph API Query                       │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Backend constructs Graph API query:                    │
│                                                           │
│  ┌────────────────────────────────────────────────┐     │
│  │ GraphService.searchEmails()                    │     │
│  │                                                 │     │
│  │ Graph API Request:                             │     │
│  │ GET /me/messages                               │     │
│  │ $filter=from/emailAddress/name eq 'Sarah'      │     │
│  │   AND contains(subject,'budget')               │     │
│  │ $orderby=receivedDateTime desc                 │     │
│  │ $top=10                                        │     │
│  │ $select=id,subject,from,receivedDateTime,      │     │
│  │         bodyPreview,hasAttachments             │     │
│  └────────────────────────────────────────────────┘     │
│                                                           │
│  Response Time: 280ms                                    │
│  Results Found: 3 emails                                 │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Results Display                                 │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  📧 Found 3 emails from Sarah about budget        ║  │
│  ║                                                    ║  │
│  ║  ┌──────────────────────────────────────────────┐ ║  │
│  ║  │ 1. Q4 Budget Review - URGENT                 │ ║  │
│  ║  │    From: Sarah Johnson                       │ ║  │
│  ║  │    Date: Nov 3, 2025 8:45 AM                 │ ║  │
│  ║  │    📎 Attachment: Q4-Budget-Draft.xlsx       │ ║  │
│  ║  │                                               │ ║  │
│  ║  │    We need to finalize Q4 budget allocations │ ║  │
│  ║  │    before today's client demo...             │ ║  │
│  ║  │                                               │ ║  │
│  ║  │    [Read Full Email] [Reply] [Forward]       │ ║  │
│  ║  └──────────────────────────────────────────────┘ ║  │
│  ║                                                    ║  │
│  ║  ┌──────────────────────────────────────────────┐ ║  │
│  ║  │ 2. Budget Planning Meeting Notes             │ ║  │
│  ║  │    From: Sarah Johnson                       │ ║  │
│  ║  │    Date: Nov 1, 2025 3:20 PM                 │ ║  │
│  ║  │                                               │ ║  │
│  ║  │    Here are the notes from our budget        │ ║  │
│  ║  │    planning meeting...                       │ ║  │
│  ║  │                                               │ ║  │
│  ║  │    [Read Full Email] [Reply] [Forward]       │ ║  │
│  ║  └──────────────────────────────────────────────┘ ║  │
│  ║                                                    ║  │
│  ║  ┌──────────────────────────────────────────────┐ ║  │
│  ║  │ 3. FW: Budget Approval Process               │ ║  │
│  ║  │    From: Sarah Johnson                       │ ║  │
│  ║  │    Date: Oct 28, 2025 10:15 AM               │ ║  │
│  ║  │                                               │ ║  │
│  ║  │    Forwarding the budget approval process... │ ║  │
│  ║  │                                               │ ║  │
│  ║  │    [Read Full Email] [Reply] [Forward]       │ ║  │
│  ║  └──────────────────────────────────────────────┘ ║  │
│  ║                                                    ║  │
│  ║  [Refine Search] [Open in Outlook]                ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ User clicks "Read Full Email"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Email Details View                              │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Tool: get_email                                         │
│  Params: { emailId: "email-id-123" }                     │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  📧 Q4 Budget Review - URGENT                     ║  │
│  ║                                                    ║  │
│  ║  From: Sarah Johnson <sarah.johnson@company.com>  ║  │
│  ║  To: John Doe <john.doe@company.com>             ║  │
│  ║  CC: Mike Williams <mike.w@company.com>          ║  │
│  ║  Date: November 3, 2025 8:45 AM                  ║  │
│  ║  Importance: High                                 ║  │
│  ║                                                    ║  │
│  ║  ──────────────────────────────────────────       ║  │
│  ║                                                    ║  │
│  ║  Hi John,                                         ║  │
│  ║                                                    ║  │
│  ║  We need to finalize Q4 budget allocations        ║  │
│  ║  before today's client demo with Acme Corp.       ║  │
│  ║  Can you review the attached spreadsheet and      ║  │
│  ║  confirm by 10 AM?                                ║  │
│  ║                                                    ║  │
│  ║  Key changes from last version:                   ║  │
│  ║  • Marketing budget increased by 15%              ║  │
│  ║  • Engineering headcount adjusted                 ║  │
│  ║  • New product development allocation added       ║  │
│  ║                                                    ║  │
│  ║  Let me know if you have questions.               ║  │
│  ║                                                    ║  │
│  ║  Thanks,                                          ║  │
│  ║  Sarah                                            ║  │
│  ║                                                    ║  │
│  ║  ──────────────────────────────────────────       ║  │
│  ║                                                    ║  │
│  ║  📎 Attachments:                                  ║  │
│  ║  • Q4-Budget-Draft.xlsx (245 KB)                 ║  │
│  ║    [Download] [Preview]                          ║  │
│  ║                                                    ║  │
│  ║  [Reply] [Reply All] [Forward] [Archive]          ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/mcp/tools/email-tools.service.ts
async searchEmails(
  params: SearchEmailsParams,
  context: McpContext,
): Promise<McpToolResult> {
  try {
    // Build Graph API filter
    let filter = '';
    if (params.sender) {
      filter += `from/emailAddress/name eq '${params.sender}'`;
    }
    if (params.keywords && params.keywords.length > 0) {
      const keywordFilter = params.keywords
        .map(k => `contains(subject,'${k}')`)
        .join(' OR ');
      filter += filter ? ` AND (${keywordFilter})` : keywordFilter;
    }

    // Execute search
    const emails = await this.graphService.searchEmails(
      context.userToken,
      {
        filter,
        maxResults: params.maxResults || 10,
        orderBy: 'receivedDateTime desc',
      },
    );

    // Format results
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({
          status: 'success',
          count: emails.length,
          emails: emails.map(e => ({
            id: e.id,
            subject: e.subject,
            from: e.from?.emailAddress,
            receivedDateTime: e.receivedDateTime,
            bodyPreview: e.bodyPreview,
            hasAttachments: e.hasAttachments,
          })),
        }, null, 2),
      }],
    };
  } catch (error) {
    this.logger.error('Error searching emails', error);
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({ status: 'error', message: error.message }),
      }],
      isError: true,
    };
  }
}
```

---

## Email Automation Flows

### Flow 3.1: Smart Email Reply

**Objective:** Draft contextual email responses using AI.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│  selects    │
│   email     │
└──────┬──────┘
       │ User: "Help me reply to this"
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Email Context Analysis                          │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent retrieves full email content:                    │
│                                                           │
│  Tool: get_email                                         │
│  Params: { emailId: "email-id-123" }                     │
│                                                           │
│  ┌─────────────────────────────────────────┐            │
│  │ Email Analysis                          │            │
│  ├─────────────────────────────────────────┤            │
│  │ From: Sarah Johnson                     │            │
│  │ Subject: Q4 Budget Review - URGENT      │            │
│  │ Sentiment: Urgent, Professional         │            │
│  │ Action Required: Review & Confirm       │            │
│  │ Deadline: 10 AM (in 45 minutes)         │            │
│  │ Key Topics: ["budget", "demo", "Q4"]    │            │
│  │ Attachments: Yes (spreadsheet)          │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Context Gathering                               │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent gathers relevant context:                        │
│                                                           │
│  1. Check user's calendar for conflicts:                │
│     Tool: get_calendar_events                            │
│     Result: 11 AM client demo with Acme Corp            │
│                                                           │
│  2. Search for related emails:                           │
│     Tool: search_emails                                  │
│     Query: "subject:budget from:sarah"                   │
│     Result: 2 previous emails in thread                  │
│                                                           │
│  3. Check knowledge base:                                │
│     Tool: search_knowledge_base                          │
│     Query: "Q4 budget approval process"                  │
│     Result: Budget approval workflow doc                 │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Draft Reply Generation                          │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent generates contextual reply:                      │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  💬 Draft Reply                                   ║  │
│  ║                                                    ║  │
│  ║  I've drafted a response based on:                ║  │
│  ║  • The email content and urgency                  ║  │
│  ║  • Your upcoming demo at 11 AM                    ║  │
│  ║  • Previous budget discussions                    ║  │
│  ║                                                    ║  │
│  ║  ──────────────────────────────────────────       ║  │
│  ║                                                    ║  │
│  ║  Hi Sarah,                                        ║  │
│  ║                                                    ║  │
│  ║  Thanks for sending this over. I'm reviewing the  ║  │
│  ║  Q4 budget spreadsheet now and will have          ║  │
│  ║  feedback by 10 AM as requested.                  ║  │
│  ║                                                    ║  │
│  ║  Quick question on the marketing budget increase: ║  │
│  ║  Is this aligned with the product launch timeline ║  │
│  ║  we discussed last week?                          ║  │
│  ║                                                    ║  │
│  ║  I'll incorporate any final changes before our    ║  │
│  ║  client demo at 11 AM.                            ║  │
│  ║                                                    ║  │
│  ║  Best,                                            ║  │
│  ║  John                                             ║  │
│  ║                                                    ║  │
│  ║  ──────────────────────────────────────────       ║  │
│  ║                                                    ║  │
│  ║  [Send Now] [Edit Draft] [Add CC] [Discard]       ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ User clicks "Edit Draft"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Interactive Editing                             │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  User provides feedback:                                │
│  "Make it shorter and remove the question"              │
│                                                           │
│  Agent refines draft:                                   │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  💬 Revised Draft                                 ║  │
│  ║                                                    ║  │
│  ║  Hi Sarah,                                        ║  │
│  ║                                                    ║  │
│  ║  Thanks for sending this. I'm reviewing the Q4    ║  │
│  ║  budget now and will confirm by 10 AM.            ║  │
│  ║                                                    ║  │
│  ║  Best,                                            ║  │
│  ║  John                                             ║  │
│  ║                                                    ║  │
│  ║  ──────────────────────────────────────────       ║  │
│  ║                                                    ║  │
│  ║  [Send Now] [Edit Again] [Discard]                ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ User clicks "Send Now"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 5: Send Email                                      │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Tool: reply_to_email                                    │
│  Params: {                                               │
│    emailId: "email-id-123",                              │
│    replyText: "Hi Sarah,\n\nThanks for sending..."      │
│  }                                                       │
│                                                           │
│  Microsoft Graph API:                                    │
│  POST /me/messages/{id}/reply                            │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  ✅ Email sent successfully!                      ║  │
│  ║                                                    ║  │
│  ║  Your reply has been sent to Sarah Johnson.       ║  │
│  ║                                                    ║  │
│  ║  I've also:                                        ║  │
│  ║  • Marked the original email as read              ║  │
│  ║  • Added a reminder to follow up at 10 AM         ║  │
│  ║  • Saved the budget attachment to your OneDrive   ║  │
│  ║                                                    ║  │
│  ║  [Dismiss] [View Sent Email]                      ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/agent.service.ts
async generateEmailReply(
  emailId: string,
  context: McpContext,
): Promise<string> {
  // 1. Get email content
  const email = await this.executeTool('get_email', { emailId }, context);

  // 2. Gather related context
  const [relatedEmails, userCalendar, knowledgeBase] = await Promise.all([
    this.executeTool('search_emails', {
      query: `subject:${email.subject.split(' ')[0]}`,
      maxResults: 3,
    }, context),
    this.executeTool('get_calendar_events', {
      startDate: new Date().toISOString(),
      endDate: new Date(Date.now() + 86400000).toISOString(),
    }, context),
    this.searchKnowledgeBase(email.subject),
  ]);

  // 3. Build context for LLM
  const llmContext = {
    email,
    relatedEmails,
    userCalendar,
    knowledgeBase,
    userPreferences: this.getUserPreferences(context.userId),
  };

  // 4. Generate reply using GPT-4o
  const reply = await this.lettaClient.sendMessage(
    this.getAgentId(context.userId),
    {
      role: 'user',
      content: `Generate a professional email reply to this email: ${JSON.stringify(llmContext)}`,
    },
  );

  return reply.content;
}
```

---

### Flow 3.2: Email Categorization and Auto-filing

**Objective:** Automatically categorize and file emails based on content.

**Flow Diagram:**

```
┌─────────────┐
│   System    │
│  monitors   │
│   inbox     │
└──────┬──────┘
       │ New email arrives
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Email Received Event                            │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Microsoft Graph webhook notification:                  │
│                                                           │
│  ┌─────────────────────────────────────────┐            │
│  │ Webhook Payload                         │            │
│  ├─────────────────────────────────────────┤            │
│  │ resourceType: "message"                 │            │
│  │ changeType: "created"                   │            │
│  │ userId: "user-123"                      │            │
│  │ messageId: "msg-789"                    │            │
│  │ timestamp: "2025-11-03T10:30:00Z"       │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Content Analysis                                │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent retrieves and analyzes email:                    │
│                                                           │
│  Tool: get_email                                         │
│  Params: { emailId: "msg-789" }                          │
│                                                           │
│  Email Content:                                          │
│  ┌─────────────────────────────────────────┐            │
│  │ From: recruiter@techcorp.com            │            │
│  │ Subject: Follow-up: Software Engineer   │            │
│  │          Position                       │            │
│  │ Body: "Thank you for applying to the    │            │
│  │        Software Engineer position at     │            │
│  │        TechCorp..."                      │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
│  AI Classification:                                      │
│  ┌─────────────────────────────────────────┐            │
│  │ Category: "Recruitment"                 │            │
│  │ Confidence: 0.92                        │            │
│  │ Priority: "Medium"                      │            │
│  │ Action Required: "Response expected"    │            │
│  │ Sentiment: "Positive"                   │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Category Decision                               │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Check user's folder structure:                         │
│                                                           │
│  Existing Folders:                                       │
│  • Inbox                                                 │
│  • Sent Items                                            │
│  • Archive                                               │
│  • Job Opportunities ← MATCH                             │
│  • Projects                                              │
│  • Personal                                              │
│                                                           │
│  Decision: Move to "Job Opportunities" folder           │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Apply Labels and Move                           │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Tool: apply_email_labels                                │
│  Params: {                                               │
│    emailId: "msg-789",                                   │
│    labels: ["Recruitment", "Action Required"],           │
│    folder: "Job Opportunities"                           │
│  }                                                       │
│                                                           │
│  Microsoft Graph API operations:                         │
│  1. POST /me/messages/{id}/move                          │
│     { destinationId: "folder-job-opps" }                 │
│  2. PATCH /me/messages/{id}                              │
│     { categories: ["Recruitment"] }                      │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 5: User Notification (Optional)                    │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  If email is high priority, notify user:                │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  📬 New Email Organized                           ║  │
│  ║                                                    ║  │
│  ║  I've filed a recruitment email for you:          ║  │
│  ║                                                    ║  │
│  ║  From: TechCorp Recruiter                         ║  │
│  ║  Subject: Follow-up: Software Engineer Position   ║  │
│  ║  Category: Recruitment                            ║  │
│  ║  Folder: Job Opportunities                        ║  │
│  ║                                                    ║  │
│  ║  This looks like a follow-up to a job application.║  │
│  ║  Would you like me to draft a response?           ║  │
│  ║                                                    ║  │
│  ║  [Draft Response] [View Email] [Dismiss]          ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/email-auto-categorizer.service.ts
@Injectable()
export class EmailAutoCategorizer {
  async categorizeEmail(emailId: string, userId: string): Promise<void> {
    // 1. Get email content
    const email = await this.graphService.getEmail(userToken, emailId);

    // 2. Classify using GPT-4o
    const classification = await this.classifyEmail(email);

    // 3. Determine target folder
    const targetFolder = await this.determineFolder(userId, classification);

    // 4. Move email and apply labels
    await this.graphService.moveEmail(userToken, emailId, targetFolder);
    await this.graphService.applyCategories(
      userToken,
      emailId,
      classification.categories,
    );

    // 5. Notify user if high priority
    if (classification.priority === 'high') {
      await this.notifyUser(userId, email, classification);
    }
  }

  private async classifyEmail(email: any): Promise<EmailClassification> {
    const prompt = `
      Classify this email into one of these categories:
      - Work: Project updates, meetings, internal communications
      - Recruitment: Job opportunities, interview requests
      - Personal: Personal emails, social notifications
      - Marketing: Newsletters, promotional emails
      - Financial: Invoices, payment confirmations
      - Other: Everything else

      Email:
      From: ${email.from}
      Subject: ${email.subject}
      Body: ${email.body}

      Return JSON: {
        "category": "...",
        "confidence": 0.0-1.0,
        "priority": "low|medium|high",
        "actionRequired": boolean
      }
    `;

    const response = await this.openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [{ role: 'user', content: prompt }],
      response_format: { type: 'json_object' },
    });

    return JSON.parse(response.choices[0].message.content);
  }
}
```

---

## Calendar Management Flows

### Flow 4.1: Smart Meeting Scheduling

**Objective:** Find optimal meeting times and create calendar events.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│  requests   │
│  meeting    │
└──────┬──────┘
       │ "Schedule a meeting with Sarah next week for 1 hour"
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Intent Parsing                                  │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent extracts meeting parameters:                     │
│                                                           │
│  Raw Input: "Schedule a meeting with Sarah next week    │
│              for 1 hour"                                 │
│                                                           │
│  Parsed Parameters:                                      │
│  ┌─────────────────────────────────────────┐            │
│  │ attendees: ["Sarah Johnson"]            │            │
│  │ duration: 60 minutes                    │            │
│  │ timeframe: "next week" (Nov 4-8, 2025)  │            │
│  │ subject: null (to be determined)        │            │
│  │ isOnlineMeeting: null (ask user)        │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Attendee Lookup                                 │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Tool: search_people                                     │
│  Params: { query: "Sarah" }                              │
│                                                           │
│  Results:                                                │
│  ┌─────────────────────────────────────────┐            │
│  │ 1. Sarah Johnson                        │            │
│  │    Email: sarah.johnson@company.com     │            │
│  │    Department: Finance                  │            │
│  │    ✓ Frequent contact                   │            │
│  │                                          │            │
│  │ 2. Sarah Williams                       │            │
│  │    Email: sarah.w@company.com           │            │
│  │    Department: HR                       │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
│  Agent selects: Sarah Johnson (based on context)        │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Availability Check                              │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Tool: get_calendar_events (for both attendees)         │
│                                                           │
│  Params: {                                               │
│    startDate: "2025-11-04",                              │
│    endDate: "2025-11-08"                                 │
│  }                                                       │
│                                                           │
│  Availability Analysis:                                  │
│  ┌──────────────────────────────────────────────────┐   │
│  │          Mon 11/4  Tue 11/5  Wed 11/6  Thu 11/7  │   │
│  ├──────────────────────────────────────────────────┤   │
│  │ 9-10 AM    ✗         ✗         ✓         ✓       │   │
│  │ 10-11 AM   ✗         ✓         ✓         ✗       │   │
│  │ 11-12 PM   ✓         ✗         ✓         ✓       │   │
│  │ 1-2 PM     ✗         ✗         ✗         ✓       │   │
│  │ 2-3 PM     ✓         ✓         ✗         ✓       │   │
│  │ 3-4 PM     ✓         ✓         ✓         ✗       │   │
│  │ 4-5 PM     ✗         ✗         ✓         ✓       │   │
│  └──────────────────────────────────────────────────┘   │
│                                                           │
│  Best Options (both available):                          │
│  1. Wednesday 11/6 at 9:00 AM                            │
│  2. Wednesday 11/6 at 11:00 AM                           │
│  3. Wednesday 11/6 at 3:00 PM                            │
│  4. Thursday 11/7 at 9:00 AM                             │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Present Options to User                         │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  📅 Available Meeting Times                       ║  │
│  ║                                                    ║  │
│  ║  I found several times when both you and Sarah    ║  │
│  ║  are available next week:                         ║  │
│  ║                                                    ║  │
│  ║  ┌──────────────────────────────────────────────┐ ║  │
│  ║  │ Wednesday, November 6, 2025                  │ ║  │
│  ║  │                                               │ ║  │
│  ║  │ ○ 9:00 AM - 10:00 AM (Recommended)           │ ║  │
│  ║  │   Early morning - both have no conflicts     │ ║  │
│  ║  │                                               │ ║  │
│  ║  │ ○ 11:00 AM - 12:00 PM                        │ ║  │
│  ║  │   Mid-morning slot                           │ ║  │
│  ║  │                                               │ ║  │
│  ║  │ ○ 3:00 PM - 4:00 PM                          │ ║  │
│  ║  │   Afternoon slot                             │ ║  │
│  ║  └──────────────────────────────────────────────┘ ║  │
│  ║                                                    ║  │
│  ║  ┌──────────────────────────────────────────────┐ ║  │
│  ║  │ Thursday, November 7, 2025                   │ ║  │
│  ║  │                                               │ ║  │
│  ║  │ ○ 9:00 AM - 10:00 AM                         │ ║  │
│  ║  │   Early morning slot                         │ ║  │
│  ║  └──────────────────────────────────────────────┘ ║  │
│  ║                                                    ║  │
│  ║  What's the meeting about?                        ║  │
│  ║  [Budget Review] [Project Update] [Other]         ║  │
│  ║                                                    ║  │
│  ║  Should this be a Teams meeting?                  ║  │
│  ║  [Yes, online] [No, in-person] [Hybrid]           ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ User selects options
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 5: Create Calendar Event                           │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  User selections:                                        │
│  • Time: Wednesday 11/6 at 9:00 AM                       │
│  • Subject: Budget Review                                │
│  • Meeting Type: Teams meeting                           │
│                                                           │
│  Tool: create_calendar_event                             │
│  Params: {                                               │
│    subject: "Budget Review",                             │
│    startDateTime: "2025-11-06T09:00:00",                 │
│    endDateTime: "2025-11-06T10:00:00",                   │
│    attendees: ["sarah.johnson@company.com"],             │
│    isOnlineMeeting: true,                                │
│    body: "Discussion about Q4 budget allocations"        │
│  }                                                       │
│                                                           │
│  Microsoft Graph API:                                    │
│  POST /me/events                                         │
│  {                                                       │
│    "subject": "Budget Review",                           │
│    "start": { "dateTime": "2025-11-06T09:00:00" },       │
│    "end": { "dateTime": "2025-11-06T10:00:00" },         │
│    "attendees": [{                                       │
│      "emailAddress": {                                   │
│        "address": "sarah.johnson@company.com"            │
│      }                                                   │
│    }],                                                   │
│    "isOnlineMeeting": true                               │
│  }                                                       │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 6: Confirmation                                    │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  ✅ Meeting Scheduled Successfully!               ║  │
│  ║                                                    ║  │
│  ║  📅 Budget Review                                 ║  │
│  ║  🕐 Wednesday, November 6, 2025                   ║  │
│  ║     9:00 AM - 10:00 AM                            ║  │
│  ║  👤 Attendees:                                    ║  │
│  ║     • You (John Doe)                              ║  │
│  ║     • Sarah Johnson                               ║  │
│  ║  💻 Teams Meeting Link:                           ║  │
│  ║     [Join Meeting]                                ║  │
│  ║                                                    ║  │
│  ║  📧 Calendar invitations sent to all attendees    ║  │
│  ║                                                    ║  │
│  ║  💡 Reminder: You have a client demo at 11 AM     ║  │
│  ║     the same day. Want me to send prep materials? ║  │
│  ║                                                    ║  │
│  ║  [View in Calendar] [Add Agenda] [Dismiss]        ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/meeting-scheduler.service.ts
@Injectable()
export class MeetingSchedulerService {
  async scheduleSmartMeeting(
    params: MeetingRequest,
    context: McpContext,
  ): Promise<CalendarEvent> {
    // 1. Resolve attendee emails
    const attendees = await this.resolveAttendees(
      params.attendees,
      context.userToken,
    );

    // 2. Find available time slots
    const availableSlots = await this.findAvailableSlots({
      attendees: [context.userEmail, ...attendees],
      duration: params.duration,
      dateRange: params.dateRange,
      userToken: context.userToken,
    });

    // 3. Rank slots by preference
    const rankedSlots = this.rankSlots(availableSlots, {
      preferMorning: true,
      avoidEndOfDay: true,
      minimizeGaps: true,
    });

    // 4. Present top options to user
    const selectedSlot = await this.presentOptionsToUser(rankedSlots);

    // 5. Create calendar event
    const event = await this.graphService.createCalendarEvent(
      context.userToken,
      {
        subject: params.subject,
        startDateTime: selectedSlot.start,
        endDateTime: selectedSlot.end,
        attendees,
        isOnlineMeeting: params.isOnlineMeeting,
        body: params.body,
      },
    );

    return event;
  }

  private async findAvailableSlots(params: {
    attendees: string[];
    duration: number;
    dateRange: { start: Date; end: Date };
    userToken: string;
  }): Promise<TimeSlot[]> {
    // Query Microsoft Graph Schedule API
    const scheduleResponse = await this.graphService.getSchedule({
      schedules: params.attendees,
      startTime: params.dateRange.start,
      endTime: params.dateRange.end,
      availabilityViewInterval: 30, // 30-minute slots
    });

    // Find overlapping free slots
    const freeSlots = this.findOverlappingFreeSlots(
      scheduleResponse,
      params.duration,
    );

    return freeSlots;
  }
}
```

---

### Flow 4.2: Meeting Preparation Assistant

**Objective:** Help users prepare for upcoming meetings with relevant context.

**Flow Diagram:**

```
┌─────────────┐
│   System    │
│  proactive  │
│   alert     │
└──────┬──────┘
       │ 30 minutes before meeting
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Meeting Detection                               │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  System checks upcoming calendar events:                │
│                                                           │
│  Current Time: 10:30 AM                                  │
│  Next Meeting: 11:00 AM - Client Demo - Acme Corp       │
│  Time Until Meeting: 30 minutes                          │
│                                                           │
│  Trigger: Pre-meeting preparation alert                 │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Context Gathering                               │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent gathers meeting context:                         │
│                                                           │
│  1. Calendar Event Details:                              │
│     Tool: get_calendar_events                            │
│     ┌─────────────────────────────────────┐             │
│     │ Event ID: event-456                 │             │
│     │ Subject: Client Demo - Acme Corp    │             │
│     │ Start: 11:00 AM                     │             │
│     │ Duration: 1 hour                    │             │
│     │ Attendees: 5 people                 │             │
│     │ Online: Teams meeting               │             │
│     │ Organizer: John Doe                 │             │
│     └─────────────────────────────────────┘             │
│                                                           │
│  2. Related Emails:                                      │
│     Tool: search_emails                                  │
│     Query: "from:acme subject:demo"                      │
│     Results: 3 emails found                              │
│                                                           │
│  3. Related Documents:                                   │
│     Tool: search_files                                   │
│     Query: "Acme Corp demo materials"                    │
│     Results: 2 PowerPoint files found                    │
│                                                           │
│  4. Previous Meeting Notes:                              │
│     Tool: search_knowledge_base                          │
│     Query: "Acme Corp previous meetings"                 │
│     Results: 1 meeting summary from last month           │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Preparation Summary                             │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent synthesizes preparation materials:                │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  ⏰ Meeting in 30 Minutes                         ║  │
│  ║                                                    ║  │
│  ║  📅 Client Demo - Acme Corp                       ║  │
│  ║  🕐 11:00 AM - 12:00 PM                           ║  │
│  ║  💻 [Join Teams Meeting]                          ║  │
│  ║                                                    ║  │
│  ║  👥 ATTENDEES (5)                                  ║  │
│  ║  • You (Host)                                      ║  │
│  ║  • Jane Smith (Acme Corp - CEO)                   ║  │
│  ║  • Bob Wilson (Acme Corp - CTO)                   ║  │
│  ║  • Sarah (Your Finance Lead)                      ║  │
│  ║  • Mike (Your Product Manager)                    ║  │
│  ║                                                    ║  │
│  ║  📋 QUICK BRIEF                                    ║  │
│  ║  • Purpose: Product demo for Q1 2026 planning     ║  │
│  ║  • Company: Acme Corp (Manufacturing)             ║  │
│  ║  • Last Contact: October 15, 2025                 ║  │
│  ║  • Deal Size: Estimated $250K annually            ║  │
│  ║  • Decision Maker: Jane Smith (CEO)               ║  │
│  ║                                                    ║  │
│  ║  🔍 KEY CONTEXT                                    ║  │
│  ║  1. Previous Meeting (Oct 15):                    ║  │
│  ║     - Expressed interest in enterprise plan       ║  │
│  ║     - Concerned about integration with SAP        ║  │
│  ║     - Wanted to see reporting features            ║  │
│  ║                                                    ║  │
│  ║  2. Recent Email (Nov 1):                         ║  │
│  ║     Jane confirmed their Q1 budget is approved    ║  │
│  ║     and they want to move fast                    ║  │
│  ║                                                    ║  │
│  ║  3. Company Research:                             ║  │
│  ║     - 500 employees, 3 manufacturing facilities   ║  │
│  ║     - Currently using legacy system (5+ years old)║  │
│  ║     - Expanding into European market              ║  │
│  ║                                                    ║  │
│  ║  📁 DEMO MATERIALS                                 ║  │
│  ║  ✓ Acme-Demo-Deck-v3.pptx (Last updated: Nov 2)  ║  │
│  ║  ✓ SAP-Integration-Guide.pdf                      ║  │
│  ║  ✗ Demo environment - [Set Up Now]               ║  │
│  ║                                                    ║  │
│  ║  💡 TALKING POINTS                                 ║  │
│  ║  • Emphasize SAP integration capabilities         ║  │
│  ║  • Highlight advanced reporting dashboard         ║  │
│  ║  • Mention European data compliance (GDPR)        ║  │
│  ║  • Discuss implementation timeline (8-12 weeks)   ║  │
│  ║                                                    ║  │
│  ║  ❓ POTENTIAL QUESTIONS                            ║  │
│  ║  • What's the total cost of ownership?           ║  │
│  ║  • Can we do a pilot with one facility first?    ║  │
│  ║  • What support do you provide during migration? ║  │
│  ║                                                    ║  │
│  ║  [Open Demo Deck] [View Previous Notes]           ║  │
│  ║  [Set Up Demo] [Add to Notes]                     ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ User clicks "Set Up Demo"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Interactive Preparation                         │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent guides setup:                                    │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  🛠️ Demo Environment Setup                        ║  │
│  ║                                                    ║  │
│  ║  I'll help you set up the demo. Here's the        ║  │
│  ║  checklist:                                        ║  │
│  ║                                                    ║  │
│  ║  ✓ Demo account created (Acme-Demo-2025)          ║  │
│  ║  ✓ Sample data loaded (manufacturing scenario)    ║  │
│  ║  ✓ SAP integration configured                     ║  │
│  ║  ⏳ Generating sample reports...                  ║  │
│  ║                                                    ║  │
│  ║  Demo URL: https://demo.app.com/acme              ║  │
│  ║  Login: demo@acme.com                             ║  │
│  ║  Password: [Click to reveal]                      ║  │
│  ║                                                    ║  │
│  ║  I've also:                                        ║  │
│  ║  • Opened your demo deck in PowerPoint            ║  │
│  ║  • Pinned relevant docs to Teams meeting          ║  │
│  ║  • Created a real-time notes doc                  ║  │
│  ║                                                    ║  │
│  ║  Ready to go! [Join Meeting Now] [Dismiss]        ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/meeting-prep.service.ts
@Injectable()
export class MeetingPrepService {
  @Cron('*/5 * * * *') // Check every 5 minutes
  async checkUpcomingMeetings(): Promise<void> {
    const users = await this.userRepository.find({ where: { isActive: true } });

    for (const user of users) {
      const upcomingMeetings = await this.getUpcomingMeetings(
        user.id,
        30, // 30 minutes
      );

      for (const meeting of upcomingMeetings) {
        // Check if we've already sent prep for this meeting
        if (!this.hasSentPrep(meeting.id)) {
          await this.sendMeetingPrep(user.id, meeting);
        }
      }
    }
  }

  async sendMeetingPrep(
    userId: string,
    meeting: CalendarEvent,
  ): Promise<void> {
    // 1. Gather context
    const context = await this.gatherMeetingContext(userId, meeting);

    // 2. Generate prep summary
    const prepSummary = await this.generatePrepSummary(meeting, context);

    // 3. Send proactive message
    await this.teamsService.sendProactiveMessage(userId, prepSummary);

    // 4. Mark as sent
    this.markPrepSent(meeting.id);
  }

  private async gatherMeetingContext(
    userId: string,
    meeting: CalendarEvent,
  ): Promise<MeetingContext> {
    const [emails, files, notes, companyInfo] = await Promise.all([
      // Search related emails
      this.executeTool('search_emails', {
        query: this.buildEmailQuery(meeting),
        maxResults: 10,
      }),
      // Search related files
      this.executeTool('search_files', {
        query: meeting.subject,
        maxResults: 5,
      }),
      // Search meeting notes
      this.searchKnowledgeBase(`previous meetings ${meeting.organizer}`),
      // Company research
      this.lookupCompanyInfo(meeting.attendees),
    ]);

    return {
      meeting,
      emails,
      files,
      notes,
      companyInfo,
    };
  }

  private async generatePrepSummary(
    meeting: CalendarEvent,
    context: MeetingContext,
  ): Promise<string> {
    const prompt = `
      Generate a concise meeting preparation summary based on:

      Meeting: ${JSON.stringify(meeting)}
      Context: ${JSON.stringify(context)}

      Include:
      1. Meeting overview
      2. Attendee information
      3. Key context from previous interactions
      4. Relevant documents and materials
      5. Suggested talking points
      6. Potential questions to prepare for

      Format as an adaptive card for Microsoft Teams.
    `;

    const response = await this.lettaClient.sendMessage(
      this.getAgentId(userId),
      prompt,
    );

    return response.content;
  }
}
```

---

## Document Search Flows

### Flow 5.1: Intelligent Document Search

**Objective:** Find documents across OneDrive and SharePoint using natural language.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│  searches   │
│  documents  │
└──────┬──────┘
       │ "Find the Q4 planning document I worked on last month"
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Query Understanding                              │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent analyzes natural language query:                 │
│                                                           │
│  Raw Input: "Find the Q4 planning document I worked on  │
│              last month"                                 │
│                                                           │
│  Extracted Parameters:                                   │
│  ┌─────────────────────────────────────────┐            │
│  │ keywords: ["Q4", "planning"]            │            │
│  │ fileTypes: ["document"] (Word/PDF)      │            │
│  │ authorFilter: "me" (current user)       │            │
│  │ timeRange: "last month" (Oct 2025)      │            │
│  │ editFilter: "modified by me"            │            │
│  │ locations: ["OneDrive", "SharePoint"]   │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
│  Search Strategy: Keyword + Metadata filtering           │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Multi-Source Search                             │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent searches across multiple sources:                │
│                                                           │
│  ┌─────────────────────────────────────────┐            │
│  │ Source 1: OneDrive                      │            │
│  │                                          │            │
│  │ Tool: search_files                      │            │
│  │ Params: {                                │            │
│  │   query: "Q4 planning",                  │            │
│  │   filter: "lastModifiedBy eq 'me'",      │            │
│  │   dateRange: "2025-10-01:2025-10-31"     │            │
│  │ }                                        │            │
│  │ Results: 4 files found                   │            │
│  │ Status: ✓ Complete (320ms)               │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
│  ┌─────────────────────────────────────────┐            │
│  │ Source 2: SharePoint Sites              │            │
│  │                                          │            │
│  │ Tool: search_sharepoint                 │            │
│  │ Params: {                                │            │
│  │   query: "Q4 planning",                  │            │
│  │   sites: ["Team Site", "Finance"]        │            │
│  │ }                                        │            │
│  │ Results: 6 files found                   │            │
│  │ Status: ✓ Complete (450ms)               │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
│  ┌─────────────────────────────────────────┐            │
│  │ Source 3: Recent Files                  │            │
│  │                                          │            │
│  │ Tool: get_recent_files                  │            │
│  │ Params: { filter: "Q4" }                 │            │
│  │ Results: 2 files found                   │            │
│  │ Status: ✓ Complete (180ms)               │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
│  Total Results: 12 files                                 │
│  Deduplicated: 10 unique files                           │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Relevance Ranking                               │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent ranks results by relevance:                      │
│                                                           │
│  Ranking Factors:                                        │
│  • Keyword match in title (weight: 0.3)                  │
│  • User is author/last editor (weight: 0.25)             │
│  • Recent modification date (weight: 0.2)                │
│  • File type preference (weight: 0.15)                   │
│  • Frequency of access (weight: 0.1)                     │
│                                                           │
│  Top Results:                                            │
│  ┌────────────────────────────────────────────┐         │
│  │ 1. Q4-Strategic-Planning-Draft.docx        │         │
│  │    Score: 0.92                             │         │
│  │    Reason: Exact match, edited by you      │         │
│  │            last week                       │         │
│  │                                             │         │
│  │ 2. Q4_Planning_Meeting_Notes.docx          │         │
│  │    Score: 0.85                             │         │
│  │    Reason: Strong match, modified in Oct   │         │
│  │                                             │         │
│  │ 3. 2025-Q4-Budget-Plan.xlsx                │         │
│  │    Score: 0.78                             │         │
│  │    Reason: Related file, collaborative     │         │
│  └────────────────────────────────────────────┘         │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Present Results                                 │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  🔍 Found 10 documents matching "Q4 planning"     ║  │
│  ║                                                    ║  │
│  ║  ⭐ TOP MATCH                                      ║  │
│  ║  ┌──────────────────────────────────────────────┐ ║  │
│  ║  │ 📄 Q4-Strategic-Planning-Draft.docx          │ ║  │
│  ║  │                                               │ ║  │
│  ║  │ 📍 OneDrive > Documents > Strategy           │ ║  │
│  ║  │ ✏️ Last edited by you on Oct 28, 2025        │ ║  │
│  ║  │ 📏 Size: 2.4 MB • 35 pages                   │ ║  │
│  ║  │                                               │ ║  │
│  ║  │ This appears to be your main Q4 planning     │ ║  │
│  ║  │ document with strategy and goals.            │ ║  │
│  ║  │                                               │ ║  │
│  ║  │ [Open in Word] [Open in Browser]             │ ║  │
│  ║  │ [Share] [Download] [Show Location]           │ ║  │
│  ║  └──────────────────────────────────────────────┘ ║  │
│  ║                                                    ║  │
│  ║  OTHER RESULTS (9)                                 ║  │
│  ║                                                    ║  │
│  ║  📄 Q4_Planning_Meeting_Notes.docx                ║  │
│  ║     Last edited: Oct 25, 2025 • SharePoint        ║  │
│  ║     [Open]                                        ║  │
│  ║                                                    ║  │
│  ║  📊 2025-Q4-Budget-Plan.xlsx                      ║  │
│  ║     Last edited: Oct 22, 2025 • Shared with 3     ║  │
│  ║     [Open]                                        ║  │
│  ║                                                    ║  │
│  ║  📄 Q4-Objectives-Template.docx                   ║  │
│  ║     Last edited: Oct 15, 2025 • OneDrive          ║  │
│  ║     [Open]                                        ║  │
│  ║                                                    ║  │
│  ║  [Show All 9 Results] [Refine Search]             ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ User clicks "Open in Word"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 5: Document Access                                 │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Tool: get_file_download_info                            │
│  Params: { fileId: "file-789" }                          │
│                                                           │
│  Microsoft Graph API:                                    │
│  GET /me/drive/items/{fileId}                            │
│                                                           │
│  Response:                                               │
│  {                                                       │
│    "webUrl": "https://company.sharepoint.com/...",      │
│    "downloadUrl": "https://download.url...",            │
│    "name": "Q4-Strategic-Planning-Draft.docx",          │
│    "size": 2457600                                      │
│  }                                                       │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  🚀 Opening Document                              ║  │
│  ║                                                    ║  │
│  ║  Q4-Strategic-Planning-Draft.docx                 ║  │
│  ║                                                    ║  │
│  ║  Opening in Microsoft Word...                     ║  │
│  ║                                                    ║  │
│  ║  💡 Tip: I can help you summarize this document   ║  │
│  ║  or extract key information. Just ask!            ║  │
│  ║                                                    ║  │
│  ║  [Summarize Document] [Extract Action Items]      ║  │
│  ║  [Dismiss]                                        ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/mcp/tools/file-tools.service.ts
async searchFiles(
  params: SearchFilesParams,
  context: McpContext,
): Promise<McpToolResult> {
  try {
    // 1. Parse natural language query
    const searchParams = await this.parseSearchQuery(params.query);

    // 2. Build Graph API queries
    const driveQuery = this.buildDriveQuery(searchParams);
    const sharePointQuery = this.buildSharePointQuery(searchParams);

    // 3. Execute searches in parallel
    const [driveFiles, sharePointFiles, recentFiles] = await Promise.all([
      this.graphService.searchDrive(context.userToken, driveQuery),
      this.graphService.searchSharePoint(context.userToken, sharePointQuery),
      this.graphService.getRecentFiles(context.userToken, {
        filter: searchParams.keywords.join(' '),
      }),
    ]);

    // 4. Merge and deduplicate results
    const allFiles = this.mergeAndDeduplicate([
      ...driveFiles,
      ...sharePointFiles,
      ...recentFiles,
    ]);

    // 5. Rank by relevance
    const rankedFiles = this.rankFilesByRelevance(
      allFiles,
      searchParams,
      context.userId,
    );

    // 6. Return formatted results
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({
          status: 'success',
          count: rankedFiles.length,
          files: rankedFiles.slice(0, 10).map(f => ({
            id: f.id,
            name: f.name,
            path: f.parentReference?.path,
            size: f.size,
            lastModifiedDateTime: f.lastModifiedDateTime,
            lastModifiedBy: f.lastModifiedBy?.user?.displayName,
            webUrl: f.webUrl,
            relevanceScore: f.relevanceScore,
          })),
        }, null, 2),
      }],
    };
  } catch (error) {
    this.logger.error('Error searching files', error);
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({ status: 'error', message: error.message }),
      }],
      isError: true,
    };
  }
}

private rankFilesByRelevance(
  files: DriveItem[],
  searchParams: SearchParams,
  userId: string,
): (DriveItem & { relevanceScore: number })[] {
  return files.map(file => {
    let score = 0;

    // Title match (weight: 0.3)
    const titleMatch = searchParams.keywords.some(kw =>
      file.name.toLowerCase().includes(kw.toLowerCase()),
    );
    if (titleMatch) score += 0.3;

    // User is author/editor (weight: 0.25)
    if (file.lastModifiedBy?.user?.id === userId) score += 0.25;

    // Recent modification (weight: 0.2)
    const daysSinceModified = (Date.now() - new Date(file.lastModifiedDateTime).getTime()) / (1000 * 60 * 60 * 24);
    if (daysSinceModified < 7) score += 0.2;
    else if (daysSinceModified < 30) score += 0.15;
    else if (daysSinceModified < 90) score += 0.1;

    // File type preference (weight: 0.15)
    const preferredTypes = ['.docx', '.xlsx', '.pptx', '.pdf'];
    if (preferredTypes.some(ext => file.name.endsWith(ext))) score += 0.15;

    // Access frequency (weight: 0.1)
    // This would require tracking user access patterns
    // For now, use view count if available
    if (file.viewCount > 10) score += 0.1;

    return { ...file, relevanceScore: score };
  }).sort((a, b) => b.relevanceScore - a.relevanceScore);
}
```

---

### Flow 5.2: Document Summarization

**Objective:** Generate summaries of documents to help users quickly understand content.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│   requests  │
│  summary    │
└──────┬──────┘
       │ "Summarize this document"
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Document Retrieval                              │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Tool: get_file_download_info                            │
│  Params: { fileId: "file-789" }                          │
│                                                           │
│  Microsoft Graph API:                                    │
│  GET /me/drive/items/{fileId}/content                    │
│                                                           │
│  Document Details:                                       │
│  ┌─────────────────────────────────────────┐            │
│  │ Name: Q4-Strategic-Planning-Draft.docx  │            │
│  │ Size: 2.4 MB (35 pages)                 │            │
│  │ Format: Microsoft Word                  │            │
│  │ Content: Text + Tables + Images         │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Content Extraction                              │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent extracts text content from document:             │
│                                                           │
│  ┌─────────────────────────────────────────┐            │
│  │ Text Extraction Results                 │            │
│  ├─────────────────────────────────────────┤            │
│  │ Total words: ~15,000                    │            │
│  │ Paragraphs: 120                         │            │
│  │ Sections: 8                             │            │
│  │ Tables: 5                               │            │
│  │ Images: 12                              │            │
│  │                                          │            │
│  │ Sections Identified:                    │            │
│  │ 1. Executive Summary                    │            │
│  │ 2. Market Analysis                      │            │
│  │ 3. Strategic Objectives                 │            │
│  │ 4. Financial Projections                │            │
│  │ 5. Product Roadmap                      │            │
│  │ 6. Go-to-Market Strategy                │            │
│  │ 7. Risk Assessment                      │            │
│  │ 8. Conclusion & Next Steps              │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: AI Summarization                                │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Processing with GPT-4o:                                │
│                                                           │
│  ⏳ Analyzing document structure...                      │
│  ⏳ Extracting key points...                             │
│  ⏳ Generating executive summary...                      │
│  ⏳ Identifying action items...                          │
│  ✓ Summary complete (2.5 seconds)                        │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Present Summary                                 │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  📄 Document Summary                              ║  │
│  ║                                                    ║  │
│  ║  Q4-Strategic-Planning-Draft.docx                 ║  │
│  ║  35 pages • Last edited Oct 28, 2025              ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  📌 EXECUTIVE SUMMARY                             ║  │
│  ║                                                    ║  │
│  ║  This Q4 strategic planning document outlines     ║  │
│  ║  the company's priorities for the final quarter   ║  │
│  ║  of 2025. Key focus areas include:                ║  │
│  ║                                                    ║  │
│  ║  • Expanding into European markets                ║  │
│  ║  • Launching two new product features             ║  │
│  ║  • Increasing revenue by 35% vs Q3                ║  │
│  ║  • Hiring 15 additional team members              ║  │
│  ║                                                    ║  │
│  ║  💰 FINANCIAL HIGHLIGHTS                          ║  │
│  ║  • Projected Revenue: $12.5M                      ║  │
│  ║  • Budget Allocation: $4.2M                       ║  │
│  ║  • Expected Profit Margin: 42%                    ║  │
│  ║  • Customer Growth Target: 250 new customers      ║  │
│  ║                                                    ║  │
│  ║  🎯 KEY OBJECTIVES (Top 5)                        ║  │
│  ║  1. Launch European sales operations (Dec 1)      ║  │
│  ║  2. Release Advanced Analytics feature (Nov 15)   ║  │
│  ║  3. Complete SAP integration (Dec 15)             ║  │
│  ║  4. Achieve 95% customer satisfaction score       ║  │
│  ║  5. Secure 3 enterprise partnerships              ║  │
│  ║                                                    ║  │
│  ║  🚀 PRODUCT ROADMAP                               ║  │
│  ║  • Advanced Reporting Dashboard (Nov 15)          ║  │
│  ║  • Mobile App iOS/Android (Dec 1)                 ║  │
│  ║  • API v2.0 Release (Dec 20)                      ║  │
│  ║  • Enterprise Security Features (Q1 2026)         ║  │
│  ║                                                    ║  │
│  ║  ⚠️ RISKS & MITIGATION                            ║  │
│  ║  • Market Competition: Focus on differentiation   ║  │
│  ║  • Resource Constraints: Prioritize hiring        ║  │
│  ║  • Technical Complexity: Incremental rollout      ║  │
│  ║                                                    ║  │
│  ║  ✅ ACTION ITEMS (8 identified)                   ║  │
│  ║  1. Finalize European market research by Nov 10   ║  │
│  ║  2. Complete product demo for enterprise clients  ║  │
│  ║  3. Hire European sales director by Nov 20        ║  │
│  ║  4. Set up EU data center infrastructure          ║  │
│  ║  5. Review and approve Q4 marketing budget        ║  │
│  ║  6. Schedule stakeholder review meeting           ║  │
│  ║  7. Update investor presentation deck             ║  │
│  ║  8. Prepare board meeting materials               ║  │
│  ║                                                    ║  │
│  ║  [View Full Document] [Export Summary]            ║  │
│  ║  [Create Tasks from Action Items]                 ║  │
│  ║  [Schedule Review Meeting]                        ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ User clicks "Create Tasks from Action Items"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 5: Action Item Conversion                          │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent creates tasks in Microsoft Planner/To Do:        │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  ✅ Creating Tasks                                ║  │
│  ║                                                    ║  │
│  ║  I'm creating 8 tasks from the action items:      ║  │
│  ║                                                    ║  │
│  ║  ✓ Task 1: Finalize European market research     ║  │
│  ║    Due: Nov 10, 2025                              ║  │
│  ║    Assigned to: You                               ║  │
│  ║                                                    ║  │
│  ║  ✓ Task 2: Complete product demo                 ║  │
│  ║    Due: Nov 12, 2025                              ║  │
│  ║    Assigned to: You                               ║  │
│  ║                                                    ║  │
│  ║  ⏳ Creating remaining 6 tasks...                 ║  │
│  ║                                                    ║  │
│  ║  All tasks created successfully!                  ║  │
│  ║                                                    ║  │
│  ║  [View Tasks in Planner] [Add to Calendar]        ║  │
│  ║  [Dismiss]                                        ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/document-summarizer.service.ts
@Injectable()
export class DocumentSummarizerService {
  async summarizeDocument(
    fileId: string,
    context: McpContext,
  ): Promise<DocumentSummary> {
    // 1. Download document content
    const fileContent = await this.graphService.getFileContent(
      context.userToken,
      fileId,
    );

    // 2. Extract text (handle Word, PDF, etc.)
    const extractedText = await this.extractTextFromDocument(
      fileContent,
      fileContent.mimeType,
    );

    // 3. Analyze document structure
    const structure = this.analyzeDocumentStructure(extractedText);

    // 4. Generate summary using GPT-4o
    const summary = await this.generateSummary(extractedText, structure);

    // 5. Extract action items
    const actionItems = await this.extractActionItems(extractedText);

    return {
      summary,
      structure,
      actionItems,
      metadata: {
        pageCount: structure.sections.length,
        wordCount: extractedText.split(/\s+/).length,
        readingTime: Math.ceil(extractedText.split(/\s+/).length / 200), // words per minute
      },
    };
  }

  private async generateSummary(
    text: string,
    structure: DocumentStructure,
  ): Promise<string> {
    const prompt = `
      Analyze and summarize this document:

      Document Structure:
      ${JSON.stringify(structure, null, 2)}

      Content:
      ${text}

      Generate a comprehensive summary including:
      1. Executive summary (2-3 sentences)
      2. Key points (5-7 bullet points)
      3. Financial highlights (if any)
      4. Main objectives/goals
      5. Risks and mitigation strategies
      6. Timeline/milestones

      Format as JSON with sections.
    `;

    const response = await this.openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [{ role: 'user', content: prompt }],
      response_format: { type: 'json_object' },
      max_tokens: 2000,
    });

    return JSON.parse(response.choices[0].message.content);
  }

  private async extractActionItems(text: string): Promise<ActionItem[]> {
    const prompt = `
      Extract all action items from this document.
      Look for tasks, todos, deliverables, and commitments.

      Document:
      ${text}

      Return JSON array of action items with:
      - description: What needs to be done
      - dueDate: When it's due (if mentioned)
      - priority: high/medium/low
      - assignee: Who is responsible (if mentioned)
    `;

    const response = await this.openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [{ role: 'user', content: prompt }],
      response_format: { type: 'json_object' },
    });

    return JSON.parse(response.choices[0].message.content).actionItems;
  }
}
```

---

## Meeting Summary Flows

### Flow 6.1: Post-Meeting Summary Generation

**Objective:** Automatically generate meeting summaries and action items after meetings end.

**Flow Diagram:**

```
┌─────────────┐
│   System    │
│  detects    │
│meeting end  │
└──────┬──────┘
       │ Meeting ends
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Meeting Completion Detection                    │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Calendar event webhook notification:                   │
│                                                           │
│  ┌─────────────────────────────────────────┐            │
│  │ Event: Meeting ended                    │            │
│  │ Meeting: Client Demo - Acme Corp        │            │
│  │ Duration: 11:00 AM - 12:05 PM (65 min)  │            │
│  │ Attendees: 5 people                     │            │
│  │ Recording available: Yes                 │            │
│  │ Transcript available: Yes                │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
│  Trigger: Post-meeting summary workflow                 │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Meeting Content Retrieval                       │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent retrieves meeting artifacts:                     │
│                                                           │
│  1. Meeting Transcript:                                  │
│     Tool: get_meeting_transcript                         │
│     Source: Microsoft Teams                              │
│     Status: ✓ Retrieved (15,000 words)                   │
│                                                           │
│  2. Chat Messages:                                       │
│     Tool: get_meeting_chat                               │
│     Status: ✓ Retrieved (25 messages)                    │
│                                                           │
│  3. Shared Files:                                        │
│     Tool: get_meeting_files                              │
│     Status: ✓ Retrieved (3 files)                        │
│     - Demo-Presentation.pptx                             │
│     - Q4-Pricing-Sheet.xlsx                              │
│     - Integration-Requirements.pdf                       │
│                                                           │
│  4. Recording:                                           │
│     Status: ✓ Available                                  │
│     Link: https://teams.microsoft.com/...                │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: AI Analysis                                     │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  GPT-4o analyzes transcript and chat:                   │
│                                                           │
│  ⏳ Identifying key discussion points...                 │
│  ⏳ Extracting decisions made...                         │
│  ⏳ Finding action items...                              │
│  ⏳ Detecting follow-up questions...                     │
│  ⏳ Analyzing sentiment and engagement...                │
│  ✓ Analysis complete (8 seconds)                         │
│                                                           │
│  Analysis Results:                                       │
│  ┌─────────────────────────────────────────┐            │
│  │ Key Topics: 5 identified                │            │
│  │ Decisions Made: 3 identified             │            │
│  │ Action Items: 7 identified               │            │
│  │ Follow-ups Needed: 4 identified          │            │
│  │ Sentiment: Positive (0.85)               │            │
│  │ Engagement Level: High                   │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Summary Generation                              │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent compiles comprehensive meeting summary:          │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  📝 Meeting Summary                               ║  │
│  ║                                                    ║  │
│  ║  Client Demo - Acme Corp                          ║  │
│  ║  November 3, 2025 • 11:00 AM - 12:05 PM           ║  │
│  ║  Duration: 1 hour 5 minutes                       ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  👥 PARTICIPANTS (5)                               ║  │
│  ║  • John Doe (You) - Host                          ║  │
│  ║  • Jane Smith - Acme Corp CEO                     ║  │
│  ║  • Bob Wilson - Acme Corp CTO                     ║  │
│  ║  • Sarah Johnson - Finance Lead                   ║  │
│  ║  • Mike Williams - Product Manager                ║  │
│  ║                                                    ║  │
│  ║  📋 MEETING OVERVIEW                              ║  │
│  ║                                                    ║  │
│  ║  Product demonstration for Acme Corp focused on   ║  │
│  ║  enterprise features, SAP integration, and        ║  │
│  ║  reporting capabilities. Overall positive         ║  │
│  ║  reception with strong interest in moving forward.║  │
│  ║                                                    ║  │
│  ║  🎯 KEY DISCUSSION POINTS                         ║  │
│  ║                                                    ║  │
│  ║  1. SAP Integration                               ║  │
│  ║     - Demonstrated real-time data sync            ║  │
│  ║     - Bob confirmed this meets their requirements ║  │
│  ║     - Discussed implementation timeline (8 weeks) ║  │
│  ║                                                    ║  │
│  ║  2. Advanced Reporting Dashboard                  ║  │
│  ║     - Showcased custom report builder             ║  │
│  ║     - Jane particularly interested in exec dashbd║  │
│  ║     - Requested sample reports for review         ║  │
│  ║                                                    ║  │
│  ║  3. Pricing & Licensing                           ║  │
│  ║     - Presented enterprise pricing tier           ║  │
│  ║     - Discussed volume discounts for 500 users    ║  │
│  ║     - Sarah to review with finance team           ║  │
│  ║                                                    ║  │
│  ║  4. Implementation & Support                      ║  │
│  ║     - Timeline: 8-12 weeks full deployment        ║  │
│  ║     - Dedicated account manager included          ║  │
│  ║     - 24/7 support for enterprise tier            ║  │
│  ║                                                    ║  │
│  ║  5. Compliance & Security                         ║  │
│  ║     - GDPR compliance confirmed                   ║  │
│  ║     - SOC 2 Type II certification shared          ║  │
│  ║     - Bob satisfied with security architecture    ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  ✅ DECISIONS MADE                                ║  │
│  ║                                                    ║  │
│  ║  1. Acme Corp will proceed with pilot program     ║  │
│  ║     starting with one manufacturing facility      ║  │
│  ║                                                    ║  │
│  ║  2. Enterprise tier selected (500 user licenses)  ║  │
│  ║                                                    ║  │
│  ║  3. Target go-live date: February 1, 2026         ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  📌 ACTION ITEMS                                   ║  │
│  ║                                                    ║  │
│  ║  [John Doe]                                       ║  │
│  ║  • Send sample executive dashboard reports        ║  │
│  ║    Due: Nov 5, 2025                               ║  │
│  ║                                                    ║  │
│  ║  • Prepare detailed implementation plan           ║  │
│  ║    Due: Nov 8, 2025                               ║  │
│  ║                                                    ║  │
│  ║  • Schedule technical deep-dive with Bob          ║  │
│  ║    Due: Next week                                 ║  │
│  ║                                                    ║  │
│  ║  [Sarah Johnson]                                  ║  │
│  ║  • Finalize pricing proposal with volume discount║  │
│  ║    Due: Nov 6, 2025                               ║  │
│  ║                                                    ║  │
│  ║  [Mike Williams]                                  ║  │
│  ║  • Create pilot program timeline                  ║  │
│  ║    Due: Nov 7, 2025                               ║  │
│  ║                                                    ║  │
│  ║  • Set up demo environment for Acme pilot         ║  │
│  ║    Due: Nov 10, 2025                              ║  │
│  ║                                                    ║  │
│  ║  [Acme Corp Team]                                 ║  │
│  ║  • Jane: Review pricing with board                ║  │
│  ║    Due: Nov 12, 2025                              ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  ❓ FOLLOW-UP QUESTIONS                            ║  │
│  ║                                                    ║  │
│  ║  • Can we customize the dashboard color scheme?   ║  │
│  ║  • What's the data retention policy?              ║  │
│  ║  • Are there any additional costs for EU hosting? ║  │
│  ║  • Can we integrate with Salesforce as well?      ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  📎 ATTACHMENTS & RESOURCES                        ║  │
│  ║  • Demo-Presentation.pptx                         ║  │
│  ║  • Q4-Pricing-Sheet.xlsx                          ║  │
│  ║  • Integration-Requirements.pdf                   ║  │
│  ║  • Meeting Recording (65 minutes)                 ║  │
│  ║  • Full Transcript                                ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  🎭 SENTIMENT ANALYSIS                            ║  │
│  ║  Overall: Positive (85%)                          ║  │
│  ║  • Jane Smith: Very Positive                      ║  │
│  ║  • Bob Wilson: Positive                           ║  │
│  ║  • Engagement Level: High                         ║  │
│  ║                                                    ║  │
│  ║  💡 NEXT STEPS                                     ║  │
│  ║  1. Schedule follow-up meeting for Nov 15         ║  │
│  ║  2. Send proposal document by Nov 8               ║  │
│  ║  3. Initiate contract preparation                 ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  [Email Summary] [Create Tasks] [Schedule Follow-up]║
│  ║  [Share with Team] [Download PDF]                 ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ Auto-distribute
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 5: Distribution & Task Creation                    │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent automatically:                                    │
│                                                           │
│  1. Email summary to all participants                    │
│     ✓ Sent to 5 attendees                                │
│                                                           │
│  2. Create tasks in Planner/To Do                        │
│     ✓ Created 7 tasks with due dates and assignments    │
│                                                           │
│  3. Save summary to SharePoint                           │
│     ✓ Saved to "Meeting Notes" folder                    │
│                                                           │
│  4. Add to knowledge base                                │
│     ✓ Indexed in ChromaDB for future reference          │
│                                                           │
│  5. Schedule follow-up meeting                           │
│     ✓ Tentative meeting scheduled for Nov 15            │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  ✅ Meeting summary complete and distributed!     ║  │
│  ║                                                    ║  │
│  ║  All attendees have been emailed the summary.     ║  │
│  ║  Action items have been created and assigned.     ║  │
│  ║                                                    ║  │
│  ║  [View Summary] [View Tasks] [Dismiss]            ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/meeting-summarizer.service.ts
@Injectable()
export class MeetingSummarizerService {
  @OnEvent('meeting.ended')
  async handleMeetingEnded(event: MeetingEndedEvent): Promise<void> {
    try {
      // 1. Retrieve meeting artifacts
      const artifacts = await this.retrieveMeetingArtifacts(event.meetingId);

      // 2. Generate summary
      const summary = await this.generateMeetingSummary(artifacts);

      // 3. Distribute summary
      await this.distributeSummary(event.meetingId, summary);

      // 4. Create tasks from action items
      await this.createTasksFromActionItems(summary.actionItems);

      // 5. Save to knowledge base
      await this.saveToKnowledgeBase(summary);

    } catch (error) {
      this.logger.error('Error processing meeting summary', error);
    }
  }

  private async retrieveMeetingArtifacts(
    meetingId: string,
  ): Promise<MeetingArtifacts> {
    const meeting = await this.graphService.getCalendarEvent(userToken, meetingId);

    const [transcript, chatMessages, files, recording] = await Promise.all([
      this.graphService.getMeetingTranscript(userToken, meetingId),
      this.graphService.getMeetingChat(userToken, meetingId),
      this.graphService.getMeetingFiles(userToken, meetingId),
      this.graphService.getMeetingRecording(userToken, meetingId),
    ]);

    return {
      meeting,
      transcript,
      chatMessages,
      files,
      recording,
    };
  }

  private async generateMeetingSummary(
    artifacts: MeetingArtifacts,
  ): Promise<MeetingSummary> {
    const prompt = `
      Analyze this meeting and generate a comprehensive summary:

      Meeting: ${JSON.stringify(artifacts.meeting)}
      Transcript: ${artifacts.transcript}
      Chat Messages: ${JSON.stringify(artifacts.chatMessages)}
      Files Shared: ${JSON.stringify(artifacts.files)}

      Generate a detailed summary including:
      1. Meeting overview (2-3 sentences)
      2. Key discussion points (5-7 major topics)
      3. Decisions made (list all)
      4. Action items with assignees and due dates
      5. Follow-up questions
      6. Sentiment analysis
      7. Next steps

      Format as JSON.
    `;

    const response = await this.openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [{ role: 'user', content: prompt }],
      response_format: { type: 'json_object' },
      max_tokens: 4000,
    });

    return JSON.parse(response.choices[0].message.content);
  }

  private async distributeSummary(
    meetingId: string,
    summary: MeetingSummary,
  ): Promise<void> {
    const meeting = await this.graphService.getCalendarEvent(userToken, meetingId);
    const attendees = meeting.attendees.map(a => a.emailAddress.address);

    // Email summary to all attendees
    await this.graphService.sendEmail(userToken, {
      recipients: attendees,
      subject: `Meeting Summary: ${meeting.subject}`,
      body: this.formatSummaryAsEmail(summary),
      attachments: [{
        name: 'Meeting-Summary.pdf',
        content: await this.generatePDF(summary),
      }],
    });

    // Post to Teams channel if applicable
    if (meeting.teamsChannelId) {
      await this.teamsService.postMessage(
        meeting.teamsChannelId,
        this.formatSummaryAsCard(summary),
      );
    }

    // Save to SharePoint
    await this.graphService.createFile({
      name: `Meeting-Summary-${new Date().toISOString()}.md`,
      content: this.formatSummaryAsMarkdown(summary),
      folder: 'Meeting Notes',
    });
  }

  private async createTasksFromActionItems(
    actionItems: ActionItem[],
  ): Promise<void> {
    for (const item of actionItems) {
      await this.graphService.createTask({
        title: item.description,
        dueDateTime: item.dueDate,
        assignedTo: item.assignee,
        priority: item.priority,
        notes: `From meeting: ${item.meetingTitle}`,
      });
    }
  }
}
```

---

## Knowledge Base Interaction Flows

### Flow 7.1: Knowledge Base Query

**Objective:** Allow users to query the organizational knowledge base for information.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│   asks      │
│  question   │
└──────┬──────┘
       │ "What's our company policy on remote work?"
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Query Analysis                                  │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent analyzes user query:                             │
│                                                           │
│  Raw Input: "What's our company policy on remote work?" │
│                                                           │
│  Query Classification:                                   │
│  ┌─────────────────────────────────────────┐            │
│  │ Type: Policy Question                   │            │
│  │ Intent: Information Retrieval           │            │
│  │ Category: HR/Workplace Policy           │            │
│  │ Keywords: ["company policy", "remote    │            │
│  │            work", "work from home"]     │            │
│  │ Expected Answer: Policy document        │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Knowledge Base Search                           │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Tool: search_knowledge_base                             │
│  Backend: ChromaDB vector search                         │
│                                                           │
│  Vector Embedding:                                       │
│  1. Convert query to embeddings using text-embedding-3  │
│  2. Search ChromaDB collection "company_policies"        │
│  3. Return top 5 most similar chunks                     │
│                                                           │
│  Search Results:                                         │
│  ┌────────────────────────────────────────────┐         │
│  │ Result 1: Remote Work Policy v2.1          │         │
│  │   Similarity: 0.94                         │         │
│  │   Source: HR-Handbook-2025.pdf, pg 42      │         │
│  │   Updated: September 2025                  │         │
│  │   Chunk: "Employees may work remotely..."  │         │
│  │                                             │         │
│  │ Result 2: Hybrid Work Guidelines           │         │
│  │   Similarity: 0.88                         │         │
│  │   Source: HR-Updates-Q3.docx               │         │
│  │   Updated: July 2025                       │         │
│  │   Chunk: "Hybrid work arrangements..."     │         │
│  │                                             │         │
│  │ Result 3: Home Office Equipment Policy     │         │
│  │   Similarity: 0.82                         │         │
│  │   Source: IT-Policies.pdf, pg 15           │         │
│  │   Chunk: "Equipment for remote work..."    │         │
│  └────────────────────────────────────────────┘         │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Answer Synthesis                                │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent synthesizes answer from multiple sources:        │
│                                                           │
│  GPT-4o processes:                                       │
│  • Top 3 search results                                  │
│  • Source documents metadata                             │
│  • Company context                                       │
│  • User's role and department                            │
│                                                           │
│  ⏳ Generating comprehensive answer...                   │
│  ✓ Answer generated (1.2 seconds)                        │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Present Answer                                  │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  📚 Remote Work Policy                            ║  │
│  ║                                                    ║  │
│  ║  Based on company policy documents:               ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  📝 POLICY OVERVIEW                               ║  │
│  ║                                                    ║  │
│  ║  Employees may work remotely up to 3 days per     ║  │
│  ║  week with manager approval. Full-time remote     ║  │
│  ║  work requires VP-level approval.                 ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  ✅ REQUIREMENTS                                   ║  │
│  ║  • Reliable high-speed internet (25+ Mbps)        ║  │
│  ║  • Dedicated workspace at home                    ║  │
│  ║  • Available during core hours (10 AM - 3 PM)     ║  │
│  ║  • Company-provided laptop and equipment          ║  │
│  ║  • VPN access configured                          ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  📋 ELIGIBILITY                                    ║  │
│  ║  • Completed probation period (6 months)          ║  │
│  ║  • Good performance rating (meets expectations+)  ║  │
│  ║  • Role suitable for remote work                  ║  │
│  ║  • Manager approval obtained                      ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  🏢 OFFICE DAYS                                    ║  │
│  ║  • Minimum 2 days/week in office                  ║  │
│  ║  • Team meeting days (usually Tuesdays)           ║  │
│  ║  • Important client meetings                      ║  │
│  ║  • Company events and all-hands meetings          ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  💻 EQUIPMENT & SUPPORT                            ║  │
│  ║  • Company laptop provided                        ║  │
│  ║  • Monitor, keyboard, mouse available on request  ║  │
│  ║  • $500 annual home office stipend                ║  │
│  ║  • IT support via Teams/email                     ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  ❓ SPECIAL CASES                                  ║  │
│  ║  • International remote work: Requires legal      ║  │
│  ║    review and VP approval                         ║  │
│  ║  • Medical accommodations: Contact HR directly    ║  │
│  ║  • Temporary arrangements: Manager discretion     ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  📄 SOURCE DOCUMENTS                               ║  │
│  ║  • Remote Work Policy v2.1 (HR Handbook pg 42)    ║  │
│  ║    [View Document]                                ║  │
│  ║  • Hybrid Work Guidelines (Q3 2025 Update)        ║  │
│  ║    [View Document]                                ║  │
│  ║  • Home Office Equipment Policy (IT Policies)     ║  │
│  ║    [View Document]                                ║  │
│  ║                                                    ║  │
│  ║  ────────────────────────────────────────          ║  │
│  ║                                                    ║  │
│  ║  💡 NEXT STEPS                                     ║  │
│  ║  Want to start working remotely?                  ║  │
│  ║  1. Discuss with your manager                     ║  │
│  ║  2. Complete Remote Work Agreement form           ║  │
│  ║  3. Submit through HR portal                      ║  │
│  ║                                                    ║  │
│  ║  [Request Remote Work] [Contact HR]               ║  │
│  ║  [View Full Policy] [Ask Follow-up]               ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         │ User clicks "Request Remote Work"
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 5: Guided Action                                   │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Agent guides user through process:                     │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  📝 Remote Work Request                           ║  │
│  ║                                                    ║  │
│  ║  I'll help you submit a remote work request.      ║  │
│  ║                                                    ║  │
│  ║  Let me check your eligibility:                   ║  │
│  ║  ✓ Probation period: Completed (hired Jan 2024)   ║  │
│  ║  ✓ Performance rating: Exceeds expectations       ║  │
│  ║  ✓ Role suitability: Yes (desk-based role)        ║  │
│  ║  ⏳ Manager approval: Pending                      ║  │
│  ║                                                    ║  │
│  ║  Great! You're eligible. Here's what I need:      ║  │
│  ║                                                    ║  │
│  ║  1. How many days per week would you like to      ║  │
│  ║     work remotely?                                ║  │
│  ║     [1 day] [2 days] [3 days] [Custom]            ║  │
│  ║                                                    ║  │
│  ║  2. Which days?                                   ║  │
│  ║     [Mon] [Tue] [Wed] [Thu] [Fri]                 ║  │
│  ║                                                    ║  │
│  ║  3. Reason for request (optional):                ║  │
│  ║     [Text input]                                  ║  │
│  ║                                                    ║  │
│  ║  I'll draft an email to your manager and prepare  ║  │
│  ║  the HR form for you.                             ║  │
│  ║                                                    ║  │
│  ║  [Continue] [Cancel]                              ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/agents/knowledge-base.service.ts
@Injectable()
export class KnowledgeBaseService {
  constructor(
    private chromaService: ChromaService,
    private openaiService: OpenAIService,
  ) {}

  async queryKnowledgeBase(
    query: string,
    context: McpContext,
  ): Promise<KnowledgeBaseResult> {
    // 1. Classify query
    const classification = await this.classifyQuery(query);

    // 2. Generate embeddings
    const queryEmbedding = await this.openaiService.createEmbedding(query);

    // 3. Search ChromaDB
    const searchResults = await this.chromaService.query({
      collectionName: this.getCollectionForCategory(classification.category),
      queryEmbeddings: [queryEmbedding],
      nResults: 5,
    });

    // 4. Synthesize answer
    const answer = await this.synthesizeAnswer(query, searchResults);

    // 5. Return formatted result
    return {
      answer,
      sources: searchResults.documents[0].map((doc, idx) => ({
        content: doc,
        metadata: searchResults.metadatas[0][idx],
        similarity: searchResults.distances[0][idx],
      })),
      confidence: this.calculateConfidence(searchResults),
    };
  }

  private async synthesizeAnswer(
    query: string,
    searchResults: ChromaQueryResult,
  ): Promise<string> {
    const context = searchResults.documents[0].join('\n\n---\n\n');

    const prompt = `
      Answer this question based on the provided context:

      Question: ${query}

      Context from knowledge base:
      ${context}

      Provide a comprehensive, well-structured answer that:
      1. Directly answers the question
      2. Cites relevant sources
      3. Includes practical next steps if applicable
      4. Uses clear formatting (bullet points, sections)

      If the context doesn't contain enough information, say so.
    `;

    const response = await this.openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [{ role: 'user', content: prompt }],
      temperature: 0.7,
    });

    return response.choices[0].message.content;
  }

  private calculateConfidence(results: ChromaQueryResult): number {
    // Calculate confidence based on similarity scores
    const distances = results.distances[0];
    if (distances.length === 0) return 0;

    // Lower distance = higher similarity = higher confidence
    const avgDistance = distances.reduce((a, b) => a + b, 0) / distances.length;

    // Convert to 0-1 confidence score (inverse of distance)
    return Math.max(0, Math.min(1, 1 - avgDistance));
  }
}
```

---

## Workflow Automation Flows

### Flow 8.1: Multi-Step Workflow Execution

**Objective:** Execute complex multi-agent workflows for automated tasks.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│  triggers   │
│  workflow   │
└──────┬──────┘
       │ "Run my daily briefing workflow"
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Workflow Identification                         │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Tool: start_workflow                                    │
│  Params: {                                               │
│    workflowName: "daily_briefing",                       │
│    userId: "user-123"                                    │
│  }                                                       │
│                                                           │
│  Workflow Definition Retrieved:                          │
│  ┌─────────────────────────────────────────┐            │
│  │ Name: daily_briefing                    │            │
│  │ Description: Comprehensive daily update │            │
│  │ Stages: 5                                │            │
│  │ Estimated Duration: 2-3 minutes          │            │
│  │ Agents Involved: 3                       │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Workflow Initialization                         │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  WorkflowOrchestratorService creates workflow run:      │
│                                                           │
│  Database Record:                                        │
│  {                                                       │
│    id: "workflow-run-456",                               │
│    workflowName: "daily_briefing",                       │
│    userId: "user-123",                                   │
│    status: "PENDING",                                    │
│    currentStage: null,                                   │
│    stages: [                                             │
│      "fetch_emails",                                     │
│      "fetch_calendar",                                   │
│      "fetch_tasks",                                      │
│      "generate_insights",                                │
│      "send_briefing"                                     │
│    ],                                                    │
│    createdAt: "2025-11-03T09:00:00Z"                     │
│  }                                                       │
│                                                           │
│  ╔════════════════════════════════════════════════════╗  │
│  ║  🔄 Starting Workflow: Daily Briefing             ║  │
│  ║                                                    ║  │
│  ║  This will take 2-3 minutes.                      ║  │
│  ║  I'll notify you when it's complete.              ║  │
│  ║                                                    ║  │
│  ║  [View Progress] [Cancel Workflow]                ║  │
│  ╚════════════════════════════════════════════════════╝  │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Stage-by-Stage Execution                        │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ┌────────────────────────────────────────┐             │
│  │ Stage 1: fetch_emails                  │             │
│  ├────────────────────────────────────────┤             │
│  │ Agent: email-agent                     │             │
│  │ Tool: search_emails                    │             │
│  │ Input: {                                │             │
│  │   query: "unread",                      │             │
│  │   maxResults: 20,                       │             │
│  │   dateRange: "last 24 hours"            │             │
│  │ }                                       │             │
│  │ Status: ✓ Complete (180ms)              │             │
│  │ Output: 8 unread emails                 │             │
│  └────────────────────────────────────────┘             │
│                                                           │
│  ┌────────────────────────────────────────┐             │
│  │ Stage 2: fetch_calendar                │             │
│  ├────────────────────────────────────────┤             │
│  │ Agent: calendar-agent                  │             │
│  │ Tool: get_calendar_events              │             │
│  │ Input: {                                │             │
│  │   startDate: "2025-11-03",              │             │
│  │   endDate: "2025-11-03"                 │             │
│  │ }                                       │             │
│  │ Status: ✓ Complete (250ms)              │             │
│  │ Output: 3 meetings today                │             │
│  └────────────────────────────────────────┘             │
│                                                           │
│  ┌────────────────────────────────────────┐             │
│  │ Stage 3: fetch_tasks                   │             │
│  ├────────────────────────────────────────┤             │
│  │ Agent: task-agent                      │             │
│  │ Tool: get_user_tasks                   │             │
│  │ Input: {                                │             │
│  │   filter: "due:today OR overdue"       │             │
│  │ }                                       │             │
│  │ Status: ✓ Complete (150ms)              │             │
│  │ Output: 5 tasks (2 due today, 3 overdue)│             │
│  └────────────────────────────────────────┘             │
│                                                           │
│  ┌────────────────────────────────────────┐             │
│  │ Stage 4: generate_insights             │             │
│  ├────────────────────────────────────────┤             │
│  │ Agent: insights-agent                  │             │
│  │ Tool: analyze_workload                 │             │
│  │ Input: {                                │             │
│  │   emails: [stage1 output],              │             │
│  │   calendar: [stage2 output],            │             │
│  │   tasks: [stage3 output]                │             │
│  │ }                                       │             │
│  │ Status: ✓ Complete (1200ms)             │             │
│  │ Output: 4 insights generated            │             │
│  └────────────────────────────────────────┘             │
│                                                           │
│  ┌────────────────────────────────────────┐             │
│  │ Stage 5: send_briefing                 │             │
│  ├────────────────────────────────────────┤             │
│  │ Agent: notification-agent              │             │
│  │ Tool: send_teams_message               │             │
│  │ Input: {                                │             │
│  │   userId: "user-123",                   │             │
│  │   content: [formatted briefing]         │             │
│  │ }                                       │             │
│  │ Status: ✓ Complete (320ms)              │             │
│  │ Output: Briefing sent to user           │             │
│  └────────────────────────────────────────┘             │
│                                                           │
│  Total Workflow Time: 2.1 seconds                        │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Workflow Completion                             │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Update database:                                        │
│  {                                                       │
│    id: "workflow-run-456",                               │
│    status: "COMPLETED",                                  │
│    completedAt: "2025-11-03T09:00:02Z",                  │
│    result: {                                             │
│      emailCount: 8,                                      │
│      meetingCount: 3,                                    │
│      taskCount: 5,                                       │
│      insightsGenerated: 4                                │
│    }                                                     │
│  }                                                       │
│                                                           │
│  User receives daily briefing (see Flow 2.1)            │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

---

## Error Handling and Recovery Flows

### Flow 9.1: Token Expiration and Refresh

**Objective:** Gracefully handle expired authentication tokens.

**Flow Diagram:**

```
┌─────────────┐
│   User      │
│  makes      │
│  request    │
└──────┬──────┘
       │ "Show me my emails"
       ▼
┌─────────────────────────────────────────────────────────┐
│ Step 1: Token Validation                                │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Backend checks user's Graph API token:                 │
│                                                           │
│  Token Status:                                           │
│  ┌─────────────────────────────────────────┐            │
│  │ accessToken: "eyJ0eXAi..."              │            │
│  │ expiresAt: "2025-11-03T08:00:00Z"       │            │
│  │ currentTime: "2025-11-03T09:30:00Z"     │            │
│  │ isExpired: TRUE                          │            │
│  │ refreshToken: "0.AXEA..."                │            │
│  └─────────────────────────────────────────┘            │
│                                                           │
│  ❌ Token expired! Attempting automatic refresh...       │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 2: Automatic Token Refresh                         │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Backend calls Azure AD token endpoint:                 │
│                                                           │
│  POST https://login.microsoftonline.com/              │
│       {tenant}/oauth2/v2.0/token                         │
│                                                           │
│  Request:                                                │
│  {                                                       │
│    grant_type: "refresh_token",                          │
│    refresh_token: "0.AXEA...",                           │
│    client_id: "{MICROSOFT_APP_ID}",                      │
│    client_secret: "{MICROSOFT_APP_PASSWORD}",            │
│    scope: "User.Read Mail.Read Calendar.Read..."        │
│  }                                                       │
│                                                           │
│  Response:                                               │
│  {                                                       │
│    access_token: "eyJ0eXAi... [new token]",             │
│    refresh_token: "0.AXEA... [new refresh token]",      │
│    expires_in: 3600                                      │
│  }                                                       │
│                                                           │
│  ✓ Token refreshed successfully!                         │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 3: Update Stored Token                             │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Update agent mapping in database:                      │
│                                                           │
│  UPDATE agent_mappings                                   │
│  SET metadata = jsonb_set(                               │
│    metadata,                                             │
│    '{accessToken}',                                      │
│    '"eyJ0eXAi... [new token]"'                           │
│  )                                                       │
│  WHERE user_id = 'user-123';                             │
│                                                           │
│  ✓ Token updated in database                             │
│                                                           │
└────────────────────────┬──────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ Step 4: Retry Original Request                          │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  Retry original user request with new token:            │
│                                                           │
│  Tool: search_emails                                     │
│  Params: { query: "unread" }                             │
│  Context: { userToken: [new token] }                     │
│                                                           │
│  ✓ Request successful!                                   │
│                                                           │
│  User receives email results (transparent to user)       │
│                                                           │
└─────────────────────────────────────────────────────────┘
```

**Technical Implementation:**

```typescript
// File: backend/src/auth/token-manager.service.ts
@Injectable()
export class TokenManagerService {
  async refreshToken(userId: string): Promise<string> {
    const agentMapping = await this.agentMappingRepository.findOne({
      where: { userId },
    });

    if (!agentMapping?.metadata?.refreshToken) {
      throw new Error('No refresh token available');
    }

    try {
      const tokenResponse = await axios.post(
        `https://login.microsoftonline.com/${process.env.AZURE_TENANT_ID}/oauth2/v2.0/token`,
        new URLSearchParams({
          grant_type: 'refresh_token',
          refresh_token: agentMapping.metadata.refreshToken,
          client_id: process.env.MICROSOFT_APP_ID,
          client_secret: process.env.MICROSOFT_APP_PASSWORD,
          scope: 'User.Read Mail.Read Calendar.Read Files.Read...',
        }),
      );

      // Update stored tokens
      agentMapping.metadata.accessToken = tokenResponse.data.access_token;
      agentMapping.metadata.refreshToken = tokenResponse.data.refresh_token;
      agentMapping.metadata.tokenExpiresAt = new Date(
        Date.now() + tokenResponse.data.expires_in * 1000,
      );

      await this.agentMappingRepository.save(agentMapping);

      return tokenResponse.data.access_token;
    } catch (error) {
      this.logger.error('Error refreshing token', error);
      throw new Error('Token refresh failed - user needs to re-authenticate');
    }
  }

  async getValidToken(userId: string): Promise<string> {
    const agentMapping = await this.agentMappingRepository.findOne({
      where: { userId },
    });

    if (!agentMapping?.metadata?.accessToken) {
      throw new Error('No token available');
    }

    const tokenExpiresAt = new Date(agentMapping.metadata.tokenExpiresAt);
    const now = new Date();

    // Check if token expires in next 5 minutes
    if (tokenExpiresAt.getTime() - now.getTime() < 5 * 60 * 1000) {
      this.logger.log('Token expiring soon, refreshing...');
      return await this.refreshToken(userId);
    }

    return agentMapping.metadata.accessToken;
  }
}

// Middleware to automatically handle token refresh
@Injectable()
export class GraphApiInterceptor implements NestInterceptor {
  constructor(private tokenManager: TokenManagerService) {}

  async intercept(
    context: ExecutionContext,
    next: CallHandler,
  ): Promise<Observable<any>> {
    const request = context.switchToHttp().getRequest();
    const userId = request.user?.id;

    if (userId) {
      try {
        // Ensure token is valid before proceeding
        request.graphToken = await this.tokenManager.getValidToken(userId);
      } catch (error) {
        throw new UnauthorizedException('Please re-authenticate');
      }
    }

    return next.handle().pipe(
      catchError(async (error) => {
        // If we get 401 from Graph API, try token refresh once
        if (error.response?.status === 401 && userId) {
          try {
            request.graphToken = await this.tokenManager.refreshToken(userId);
            // Retry the request
            return next.handle();
          } catch (refreshError) {
            throw new UnauthorizedException('Please re-authenticate');
          }
        }
        throw error;
      }),
    );
  }
}
```

---

## Conclusion

This document provides comprehensive user flow documentation for the Amaiko AI enterprise assistant application. Each flow demonstrates the interaction between users, the AI agent, and various Microsoft 365 services through the MCP tools layer.

**Key Takeaways:**

1. **Seamless Authentication**: OAuth 2.0 flows with automatic token refresh ensure uninterrupted service
2. **Multi-Agent Architecture**: Specialized agents collaborate to handle complex workflows
3. **Natural Language Understanding**: GPT-4o powers intent recognition and response generation
4. **Proactive Assistance**: System-initiated notifications and workflow automation
5. **Knowledge Base Integration**: ChromaDB vector search for organizational knowledge
6. **Error Resilience**: Graceful error handling with automatic recovery
7. **Context Awareness**: Agent memory systems maintain conversation continuity

**Files Referenced:**

- `/backend/src/agents/agent.service.ts` - Core agent logic
- `/backend/src/agents/agent-memory.service.ts` - Memory management
- `/backend/src/agents/workflow-orchestrator.service.ts` - Workflow execution
- `/backend/src/mcp/tools/email-tools.service.ts` - Email MCP tools
- `/backend/src/mcp/tools/calendar-tools.service.ts` - Calendar MCP tools
- `/backend/src/graph/graph.service.ts` - Microsoft Graph integration
- `/backend/src/agents/entities/agent-mapping.entity.ts` - User-agent mappings
- `/backend/src/agents/entities/workflow-run.entity.ts` - Workflow tracking

---

**Document Version:** 1.0
**Last Updated:** November 2025
**Maintainer:** Amaiko AI Development Team
